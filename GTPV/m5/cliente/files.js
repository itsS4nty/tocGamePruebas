...
*/
	...
	my.getNames = function(registered, param) { return names[registered][param]; }
	
	my.localExec = function(names) {
		names.forEach(function(n) {
			var f = H.Web.findFile(scripts, n, "sat", true);
			if (f) {
				f.localExec();
	...
	my.addLocalExec = function(name, params, func, path, registered, type, utf8) {
		var f = my.add(name, params, func, path, registered, type, utf8);
		f.localExec();
		f.localExec = dummyFunc;
	...
	my.add = function(name, params, func, path, registered, type, utf8) {
		var f = {};
		...
		var fs = [f];
		if (typeof f.registered === "boolean") {
			fs.push($.extend({}, f));
			fs[0].registered = true;
			fs[1].registered = false;
		}
		fs.forEach(function(f) {
			...
			if (i < scripts.length) {
				...
				sC.localExec = function(sCLocalExec, fLocalExec) {
					return function() {	
						sCLocalExec();
						fLocalExec();
					};
				}(sC.localExec, f.localExec); 
			} else {
				scripts.push(f);
				if (params == null) params = " ";
				params.split("").forEach(function(p) {
					(names[f.registered][p] = names[f.registered][p] || []).push(f.name);
				});
			}
		});
		return fs[0];
