...
//		hAT = divT.height();
	...
	function restoreState() {
		...
		if (S.estado == null) {
			S.id = idComanda;
			S.C = Comandes.get(idComanda, changeComanda, function(C, codError, o, c) {
				console("error C: "+codError+" : "+o.cmd);
			}); // ???? controlHandler
			S.AC = agruparComanda(prefAgruparComanda_Venda, S.C);
			S.alertDialog = null;
			S.teclats = {};
			abrirComanda();
		}
	...
	(function() { // clousure para evitar interferencia de identificadores de la app de cobrar
		...
		var butImprimir = gButton().appendTo(divIS).click(imprimirTicket);
		var butSalir = gButton().appendTo(divIS).click(function(e){ 		
																if (e.button !== 0) return;
																S.estado="total"; 
																redraw();
														   });	
	...
	function actualizarCobrar() {
		...
		cobrarTotal.text(formatImport(S.total, true).fillSpaceL(20));
...
window.AppAbrirCaja = function() {
	...
	var divEspereInner = $("<div>")._100x100().text(M("Esperi...")).addClass('g-widget-overlay');
