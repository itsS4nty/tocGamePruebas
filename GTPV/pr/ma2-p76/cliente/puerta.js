procesoPuerta = function() {
	...
	var opSubMenu = {
		entradaSalidaDependenta : {
			...
			run: function() { procesoEntradaSalidaDependenta.start(mp); },
		...
		impresora : {
			text: "Impresora",
			run: function() { procesoImpresora.start(mp); },
...
*/	
	function obtenerDependentesNoActives() {
		...
		for (var i=0; (depT = Dependentes.getByIdx(i)) != null; i++) {
			var depA;
			for (var j=0; (depA = Dependentes.getActivaByIdx(j)) != null; j++) {
			...
			DB.preOpenMensual(sqlDate, "V_Horaris_", function(h) {
				...
				DB.transactionWithErr(db, function (tx) {
					...
					DB.sincroInsert(tx, h.tableName, "[Botiga], [Data], [Dependenta], [Operacio], ",
													 ...
													 h.mark);
				}, function() { /*DB.inTransaction(false);*/ Dependentes.addActiva(depSel); }
				);	
			});
		}
	}
