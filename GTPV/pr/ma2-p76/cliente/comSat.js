	...
	function processAnswer(answer, pendResp) {
		try {
			...
			if ((answer.er != null) && (typeof pr.errorHandler === "function")) { 
				pendResp.errorHandler(answer.er);
			}  else {
				if (typeof pr.callback === "function") 
					pendResp.callback(answer.ret);
			}
		} catch(e) {
			console.log(e);
		}
...
//			satellites.push({ idSat: idSat, idxSecHS: 1, objs: [], pendingHS: [] });
			...
			if (file != null) {
				var body = (file.ansi) ? StringToANSI(file.content) : file.content;
	...
	function sendScript(idSat, string, callback){
		return	;
	}
	...
	my.sendScriptText = function(idSat, s, callback) {
		if (typeof s === 'function') s = s.toString();
		return send(idSat, {type: "s", script: s}, callback, callback);
	}
...
//	my.setIdSat = function(_id) { idSat = _id; }
	...
	function processAnswer(answer, pendResp) {
		try {
			if ((typeof answer !== "object") || (answer == null)) answer = {};
			if ((answer.er != null) && (typeof pendResp.errorHandler === "function")) { 
				pendResp.errorHandler(answer.er);
			}  else {
	...
	my.init = function(_idSat) {
		...
		if (isLocal) localHost.localSatInit(localRecHS); // idSat == CH
