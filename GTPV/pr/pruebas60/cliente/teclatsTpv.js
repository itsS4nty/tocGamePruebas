...
//		hTeclado : 0.6,
	...
	var divAT = positionDiv(null, 0, 0, 100, 100);
...
//		divAT.height(ambientsScroll.getDiv().height()+divTBE.height());
	...
	var ambientsScroll = new myScroll("_lr", Config.nButX);
	...
	function redrawDivTeclat() {
		...
		for (var i=0; i<buttonsTeclat.length; i++) {
			...
			if ((data == null) || (data.article == null)) {
				data = { codiArticle: null, ambient: ambient, pos: i };
				var text = "";
				var bg = "";
...
*/
	...
	function clickNivellArticleHandler(e) {
		...
		if (obj == art) { // no se si puede pasar
			...
			my.selectedArticleHandler({ article: art });
	...
	function redrawDivBuscar(nivInicial) {
		...
		if (numNivells != currentNumNivells) {
			...
			for (var i=0; i<numNivells+1; i++) {
				if (nivellsScroll[i] == null) {
					nivellsScroll[i] = new myScroll("_ud", defaultElementNivells.clone(false)); 
		...
		if (niv >= nivInicial) {
			nivellsScroll[niv].removeAll();
			nivellsScroll[niv].setContentNewElement(nivellModel);
	...
	var inputEntrar = $("<input type='text'>").css({position: "absolute", margin: "1em"}).appendTo(divEntrar);
	var keyboardEntrar = new Keyboard();
	keyboardEntrar.getDiv().css({position: "absolute", margin: "1em"}).appendTo(divEntrar);
	...
	var autoCompleteEntrar = new myScroll("_ud", defaultAutoCompleteElementEntrar);
	autoCompleteEntrar.getDiv().css({position: "absolute", margin: "1em"}).appendTo(divEntrar);

	function redrawDivEntrar() {
		autoCompleteEntrar.removeAll();
		autoCompleteEntrar.setContentNewElement(modelAutoCompleteEntrar);
	...
	function resizeDivEntrar() {
		...
		positionKeyboard(keyboardEntrar, 0, h0-h0*Config.hKeyboardEntrar, w0*Config.wKeyboardEntrar, h0);
		posAbsolutePXM(autoCompleteEntrar.getDiv(), w0*Config.wKeyboardEntrar, 0, w0, h0);
		autoCompleteEntrar.redraw();
...
var DatosArticles = (function(){
	...
	my.renAmbient = function(oldAmbient, newAmbient) {
		...
		db.transaction(function(tx) {
			tx.executeSql("UPDATE TeclatsTpv SET ambient = ? WHERE ambient = ?", [oldAmbient, newAmbient]);
		});
		return true;
	}
	...
	my.delAmbient = function(ambient) {
		...
		var db = DB.openPrincipal();
		db.transaction(function(tx) {
			tx.executeSql("DELETE FROM TeclatsTpv WHERE ambient = ?", [ambient]);
		});
		return true;
	}
	...
	my.addArticleTeclat = function(but) {
		...
		var db = DB.openPrincipal();
		db.transaction(function(tx) {
			tx.executeSql("DELETE FROM TeclatsTpv WHERE (ambient = ?) AND (pos = ?)", [but.ambient, but.pos]);
			tx.executeSql("INSERT INTO TeclatsTpv (ambient, article, pos, color) VALUES (?, ?, ?, ?)",
	...
	my.changeArticleTeclat = function(but1, but2) {
		...
		db.transaction(function(tx) {
			if (but1.pos != null) {
				tx.executeSql("DELETE FROM TeclatsTpv WHERE (ambient = ?) AND (pos = ?)", [but1.ambient, but1.pos]);
				tx.executeSql("INSERT INTO TeclatsTpv (ambient, article, pos, color) VALUES (?, ?, ?, ?)",
					[but1.ambient, but2.codiArticle, but1.pos, but2.color]
				);	
			}
			if (but2.pos != null) {
				tx.executeSql("DELETE FROM TeclatsTpv WHERE (ambient = ?) AND (pos = ?)", [but2.ambient, but2.pos]);
				tx.executeSql("INSERT INTO TeclatsTpv (ambient, article, pos, color) VALUES (?, ?, ?, ?)",
	...
	my.changeColorArticleTeclat = function(but, color) {
		...
		var db = DB.openPrincipal();
		db.transaction(function(tx) {
			tx.executeSql("UPDATE TeclatsTpv SET color = ? WHERE (ambient = ?) AND (pos = ?)", 
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			...
			tx.executeSql(stat, [/*GlobalGTPV.get("Llicencia")*/], function (tx,res) {
				...
				procesarDatos();
			}, function (tx,e) { resTeclats = []; procesarDatos(); });
		...
		function procesarDatos() {
			...
			resArticles.forEach(function(el) { el.compareNom = preProcessCompareNom(el.nom) });
			...
			for (var i=0; i<resTeclats.length; i++) {
				...
				but.ambient = (but.ambient || "");
				getOrNewAmbient(but.ambient).buttons[but.pos] = but;
			}
