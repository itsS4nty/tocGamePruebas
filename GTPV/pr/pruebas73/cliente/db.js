...
*/	my.init = function(_prefijo) {
		prefijo = _prefijo;
//		my.initMensual();
		pendingSincroUD = LS.get("pendingSincroUD");
		if (!(pendingSincroUD instanceof Array)) pendingSincroUD = [];
...
*/	
		...
		function getTransaction(dbName) {
			return function(tx) {
				for (var i=0; i<pendingSincro_UD.length; ) {
					...
					if ((pendSql.dbName == dbName) && (!lockTables)) {
						my.sincroUpdateDelete_UD(dbName, tx, p.table, p.keys, p.others, p.tipo, p.mark, null, null);
						pendingSincro_UD.splice(i,1);
						LS.set("pendingSincroUD", pendingSincroUD);
					} else i++;
				}
			}
		}
	...
	function sincroUpdateDelete_UD(dbName, tx, tableName, keys, others, tipo, mark, callback, errorCallback) {
		if (lockTables) {
			pendingSincroUD.push({dbName: dbName, table: tableName, keys: keys, others: others, tipo: tipo, mark: mark});
			LS.set("pendingSincroUD", pendingSincroUD);
			callback(tx, null);
			return;
		}
