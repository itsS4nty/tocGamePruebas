...
//			Caja.set(c);
			...
			redraw();
			var hv = DB.preOpenMensual(date, "V_Venut_");
			...
			db.transaction(function (tx) {
				...
				DB.sincroDelete(tx, hv.tableName, "([Data] = ?) AND ([Num_tick] = ?)", 
				                [date, numTick], hv.mark, function() { Caja.set(c); }, function() { Caja.set(c); });
			});
		}
	}
	...
	function previewT() {
		...
		db.transaction(function (tx) {
			var stat = "SELECT [Dependenta] as dep, [Plu] as plu, [Quantitat] as cant, [Import] as import "
			          +"FROM "+hv.tableName+" WHERE ([Data] = ?) AND ([Num_tick] = ?)";
			DB.exec(tx, stat, [date, numTick], function(tx, r) {
				if (r.rows.length > 0) {
					previewTScroll.removeAll();
					var D = {};
	...
	keyboard.setCallback(function(m) {
		switch(m) {
			case "enter" :
				...
				Caja.apunte(S.tipo, (S.tipo == "O") ? -imp : imp, input.val(), dep);
	...
	function okHandler(e) {
		...
		if (imp > 0) {
			if (S.textoSelected == null) {
				...
				Caja.apunte(S.tipo, (S.tipo == "O") ? -imp : imp, S.textoSelected, dep);
				menuPrincipal.finProceso();	
			}
		}
	}
