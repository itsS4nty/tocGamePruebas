...
var DatosArticles = (function(){
	...
	my.renAmbient = function(oldAmbient, newAmbient) {
		...
		datosTeclats = datosTeclats.sort(getToLowerSortFunction("ambient"));
		var h = DB.preOpenPrincipal("TeclatsTpv");
		...
		db.transaction(function(tx) {
			...
			t.buttons.forEach(function(but) { 
				DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
					{ ambient: newAmbient, pos: but.pos },
					{ article: but.codiArticle, color: but.color, Dependenta: my.codiDepModTeclats, 
					  Data: DB.DateToSql(), llicencia: GlobalGTPV.get("Llicencia") }, h.mark);
			});
		});
		return true;
	}
	...
	my.addArticleTeclat = function(but) {
		...
		db.transaction(function(tx) {
			DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
				{ ambient: but.ambient, pos: but.pos },
				{ article: but.codiArticle, color: but.color, Dependenta: my.codiDepModTeclats, 
				  Data: DB.DateToSql(), llicencia: GlobalGTPV.get("Llicencia") }, h.mark);
		});
	}
	...
	my.delArticleTeclat = function(but) {
		...
		db.transaction(function(tx) {
			DB.sincroDelete_UD(h.dbName, tx, h.tableName, 
				{ ambient: but.ambient, pos: but.pos }, h.mark);
		});
	}
	...
	my.changeArticleTeclat = function(but1, but2) {
		...
		db.transaction(function(tx) {
			if (but1.pos != null) {
				DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
					{ ambient: but1.ambient, pos: but1.pos },
	...
	my.changeColorArticleTeclat = function(but, color) {
		...
		db.transaction(function(tx) {
			DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
				{ ambient: but.ambient, pos: but.pos },
				{ color: but.color, Dependenta: my.codiDepModTeclats, 
				  Data: DB.DateToSql(), llicencia: GlobalGTPV.get("Llicencia") }, h.mark);
		});
	}
...
//		if (idx == null) idx = idxNivell0;
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			...
			resTeclats = []; resArticles = []; resFamilies = []; resCodisBarres = []; procesarDatos();
			/*DB.errorOpenDB()*/
		} );
