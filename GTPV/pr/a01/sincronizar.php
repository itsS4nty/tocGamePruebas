...
define("PREFIX_DEFAULT_PREFIX_TABLE", "DEFAULT");
define("DEFAULT_PREFIX_TABLE", PREFIX_DEFAULT_PREFIX_TABLE."AE"."_");
...
//	global $DEFAULT_PREFIX_TABLE;
	...
	if (!odbc_execute($sql, array())) { 
		...
		$stat = "SELECT ".$top0." * INTO $tableServer FROM [".DEFAULT_PREFIX_TABLE."$table]";
		debug_str($stat);
		$sql = odbc_prepare($ConnDB, $stat); 
		if (!odbc_execute($sql, array())) { enviarODBCError("202# Select [".DEFAULT_PREFIX_TABLE."$table]"); }
...
//	global $DEFAULT_PREFIX_TABLE;	
	...
	$sql = odbc_prepare($ConnDB, $stat); 
	if (!odbc_execute($sql, array())) { 
		$stat = "SELECT TOP 0 * INTO $tableServer FROM [".DEFAULT_PREFIX_TABLE."$tableModel]";
debug_str($stat);
		$sql = odbc_prepare($ConnDB, $stat); 
		if (!odbc_execute($sql, array())) { enviarODBCError("207# SELECT INTO $tableServer FROM [".DEFAULT_PREFIX_TABLE."$tableModel]"); }
	}
}
