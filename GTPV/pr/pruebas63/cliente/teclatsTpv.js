...
//		hTeclado : 0.6,
	...
	function codiBarresHandler(art, codiBarres) {
		if (art == null) return;
...
//		if (idx == null) idx = idxNivell0;
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			...
			var stat = "SELECT Codi as codi, Producte as codiArticle ";
			DB.exec(tx, stat, [], function (tx,res) {
		...
		function procesarDatos() {
			...
			for (var i=0; i<resCodisBarres.length; i++) {
				...
				if ((cb.codi != null) && (cb.codi != "")) {
					cb.article = findArticleByCodi(cb.codi);
...
var CodisBarres = function(){
	...
	function handler(e) {
		if ((e.which >= "0") && (e.which <= "9")) {
			...
			if (buffer.length > 0) {
				var art = DatosArticles.findCodiBarres(buffer);
				if (registeredhandler != null) registeredHandler(art, buffer);
				buffer = "";
			}
		}
	}
