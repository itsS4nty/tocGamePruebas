function myScroll(arrows, nElementsScroll, options) {
	...
	var block = horiz ? "inline-block" : "block";
	...
	this.newElement = function() {
		...
		el.css(nsize, "100%").css({display : block, boxSizing : "border-box"}).appendTo(scrollContentDiv);
...
*/		return el;
	...
	this.redraw = function() {
		...
		dispA = (nElements > opt.nElementsScroll) ? block : "none";
		for (var i=0; i<=1; i++) {
			sa[i]=0;
			if (vAD[i]) {
