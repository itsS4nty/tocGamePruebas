...
function inicializarConServidor(idCliente, password, producto, version) {
	var doc = document.implementation.createDocument("", "gtpv", null);
	var init = doc.createElement("init");
	setAttribute(init, "id", idCliente);
	setAttribute(init, "password", password);
	setAttribute(init, "producto", producto);
	setAttribute(init, "version", version);
