...
//		this.redraw();
	...
	this.scrollTo = function(el, fScrollBlocks) {
		...
		if (fScrollBlocks != null) scrollInBlocs = fScrollBlocks;
		if (el >= nElements) el = nElements-1;
		if (el < 0) el = 0;
		visibleElementScroll = el;
		if (scrollInBlocks) currentTopElementScroll = el - (el%currentNumElementsScroll);
		/* scrollContentDiv.css(lot, -((el/opt.nElementsScroll)*100+0.0001)+"%"); */
		scrollContentDiv.css(lot, (-currentTopElementScroll*(contentDiv[size]()/currentNumElementsScroll))+"px");
		setStateButtons();
	}
}
