...
//		if (idx == null) idx = idxNivell0;
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			var stat = "SELECT Ambient as ambient, Article as codiArticle, Pos as pos, Color as color FROM TeclatsTpv ";
			var params = [];
			if (GlobalGTPV.get("Llicencia") != null) {
				stat += "WHERE (Llicencia = ?)";
				params.push(GlobalGTPV.get("Llicencia"));
			}
			tx.executeSql(stat, params, function (tx,res) {
		...
		function procesarDatos() {
			...
			resArticles.forEach(function(el) { if (el.nom == null) el.nom=""; });
