procesoEditorTeclats = function() {
	...
	var dialogBorrarAmbient = createAlertDialog().header("Borrar?")
	                                             .setButtons(["Ok", "Cancel"], function(text, i) { 
													if (text=="Ok") {
														DatosArticles.delAmbient(teclatsTpv.getAmbientActual(), reloadIfError);
													} 
													redrawT(true);
	                                             });
	dialogBorrarAmbient.getDiv().appendTo(div0);

	function reloadIfError() { DatosArticles.reload(); }
	...
	teclatsTpv.selectedArticleHandler = function(data, but) {
		...
		if (S.artSelected == null) {
			...
			if ((S.artSelected.pos == null) && (data.pos == null)) {
				...
				DatosArticles.changeArticleTeclat(S.artSelected, data, reloadIfError);
		...
		switch(m) {
			case "enter":
				...
				if (newAmb != null) {
					...
					if (S.estado == "addGrup") {
						DatosArticles.addAmbient(inp, reloadIfError);
						teclatsTpv.setAmbientActual(inp);
					}
					if (S.estado == "renGrup") {
						DatosArticles.renAmbient(teclatsTpv.getAmbientActual(), inp, reloadIfError);
						teclatsTpv.setAmbientActual(inp);
					}
					redrawT(true);
				}
...
//		hTeclado : 0.6,
	...
	function clickButsCol(e) {
		...
		DatosArticles.changeColorArticleTeclat(S.artSelected, color, reloadIfError);
		setArtSelected(null);
		redrawT(false);
	}
	...
	function butAccionsHandler(e) {
		...
		switch ($(this).data("data")) {
			...
			case "delArticle" :
				if (S.artSelected != null) {
					DatosArticles.delArticleTeclat(S.artSelected, reloadIfError);
					S.teclat.estado = null;
					setArtSelected(null);
					redrawT(false);
				}
...
//		teclatsTpv.redraw(S.teclat);
	...
	function redrawT(fAll) {
		...
		divAccions.children().removeClass("ui-state-active");
		if ((S.artSelected != null) && (S.artSelected.article != null) && (S.artSelected.pos != null)) {
