...
var procesoPrinter = function() {
	...
	function testSerialHandler(e) {
		...
		if ((testPrinter != null) && (testPrinter != curPrinter) && (selectedP != testPrinter.getName())) {
			testPrinter.close();
		}
		...
		remeberCloseTestPrinter();
	}
	
	function rememberCloseTestPrinter() {
		if (!isVisible(div0)) closeTestPrinter();
		else {
	...
	function selectSerialPortHandler(e) {
		...
		if (curPrinter.getName() != selectedP) {
			if (testPrinter.getName() == selectedP) {
				curPrinter.close();
