...
function createDefaultDownloadTable($Tabla, $camposIndice) {
	...
	if (!odbc_execute($sql, array())) { 
		if ($cliente['DBServidor'] !== '') { return; /*enviarODBCError("201# Select $TablaServer");*/ }
		$stat = "SELECT * INTO $TablaServer FROM [DEFAULT_$Tabla]";
		debug_str($stat);
		$sql = odbc_prepare($ConnDB, $stat); 
		if (!odbc_execute($sql, array())) { return; /*enviarODBCError("202# Select [DEFAULT_$Tabla]");*/ }
	}
...
debug_str("sincro ".$sincro);	
	...
	debug_str('sql error 10');
		...
		$sql = odbc_prepare($ConnDB, $stat); 
		if (!odbc_execute($sql, $paramsSql)) { return; /*enviarODBCError("205# $TablaServerSincro JOIN $TablaServer");*/ }
	}
...
*/	$tablasSincronia = getTablasSincronia();
	foreach($tablasSincronia as $tablaS) {
		if (($tablaS["modoSincro"] == 'download') && (count($tablaS["camposIndice"]) > 0)) 
			downloadTable($tabla["tabla"], $tabla["camposIndice"], $infoSincro[$table]);
	}
...
function requestSincroDownload_UD() {
	...
	$nodeExec = createElementOut('exec');
	setAttribute($nodeExec, 'id', 'download_UD');
	$nodeDB->appendChild($nodeExec);
	appendDocOut($nodeDB);
}

function sincroUpload($nodeExec) {
	$tablasSincro = getTablasSincronia();
	$sincroUploadDownload = FALSE;
	...
	if ($nodeColumns !== NULL) {
		...
		for ($nodeRow = getNextSiblingElement($nodeColumns, 'row'); $nodeRow !== NULL; $nodeRow = getNextSiblingElement($nodeRow, 'row')) {
			...
			foreach ($tablasSincro as $tablaS) {
				if ((($tablaS['modoSincro'] == 'uploadDownload') && ($tablaS['tabla'] == $table)) ||
				    (($tablaS['modoSincro'] == 'upload') && (strncasecmp($table, $tablaS['tabla'], strlen($tablaS['tabla'])) === 0))) {
					if (($tablaS['modoSincro'] == 'uploadDownload') && (!$sincroUploadDownload)) {	
						requestSincroDownload_UD();
						$sincroUploadDownload = TRUE;
					}
					$nodeDB = createElementOut('db');
...
function uploadTables($nodeDB, $infoSincroDownload) {
	global $ConnDB;
	global $cliente;
...
//	$infoUpload = getInfoUpload();
	...
	for ($nodeExec = getFirstChildElement($nodeDB, 'exec'); $nodeExec !== NULL; $nodeExec = getNextSiblingElement($nodeExec, 'exec')) {
		...
		foreach ($tablasSincronia as $tablaS) {
			if (strncasecmp($table, $tablaS['tabla'], strlen($infoItem['tabla'])) == 0) {
				$nodeColumns = getFirstChildElement($nodeExec, 'columns');
				if ($nodeColumns !== NULL) {
	...
	while (($nodeDB = getNextPrincipalNode('db')) != NULL) {
		switch (getAttribute($nodeDB, 'id')) {
			...
			case 'download' :
				break;
			case 'upload' :
				uploadTables($nodeDB, $infoSincroDownload);
				break;
		}
	}
