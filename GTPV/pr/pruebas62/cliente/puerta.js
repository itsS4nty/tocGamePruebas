procesoPuerta = function() {
	...
	var opSubMenu = {
		...
		impresora : {
			text: "Impresora",
			run: function() { Printer.start(); },
			init: function() { Printer.init(div02); }
		}
	};
	...
	my.start = function(_dep) {
		...
		S.opSubMenuSel = (S.opSubMenuSel || subMenus[S.idxSubMenu][0]);	
		subMenuScroll.removeAll();
		elSubMenuSel = null;
...
//	function actualizarDependentesNoActives() {	
	function obtenerDB() {
		...
		dbP.transaction(function (tx) {
			DB.exec(tx, statP, [], function (tx,res) {
				...
				if (!fHayResponsable) { depTotals.forEach(function(dep) { dep.esResponsable = true; }); }
	...
	function obtenerDependentesNoActives() {
		depNoActives = [];
		for (var i=0; i<depTotals.length; i++) {
			var dep = null;
			for (var j=0; (dep = DepActives.getByIdx(j)) != null; j++) {
				if (depTotals[i].codi == dep.codi) break;	
			}
			if (dep == null) {
				depNoActives.push(depTotals[i]);
			}
		}
	}
	...
	function callbackKeyboard1(m) {
		...
		if (m == "enter") {
			...
			var dep = {};
			for (var p in depSel) { if (depSel.hasOwnProperty(p)) { dep[p] = depSel[p]; } } 
			delete dep.password;
			delete dep.compareNom;
			DepActives.add(dep);	
		}
	}
