...
//				abrirCajon();
				...
				S.date = new Date();
				var sqlDate = DB.getSqlDate(S.date);
				var codiDep = S.codi; 
				calcularTotalTicket();
				c.sumTick += S.total;
				c.clients++;
				var h = DB.preOpenMensual(sqlDate, "V_Venuts_");
...
//		ticketScroll();
	...
	(function() {
		...
		function imprimirTicket() {
			...
			Printer.reset();
			Printer.startBuffer();
			Printer.font("B",false,1,1);
			var d = S.date;
			var dateStr = dos0(d.getDate())+"-"+dos0(d.getMonth()+1)+"-"+(d.getFullYear()%100)+" "
			             +dos0(d.getHours())+":"+dos0(d.getMinutes())+":"+dos0(d.getSeconds());
			var str = "Atès Per : "+dep.nom+" ";
			str = fillSpaceR(str, 56-dateStr.length)+dateStr+"\n";
			Printer.print(str);
	                     /*                                        1234567812345678*/ 		
			             /*12345678901234567890123456789012345678901234567890123456*/
			Printer.print("  UTS   DESCRIPCIO                        PREU   IMPORT \n");
			Printer.printLine("-",56);
			for (var i=0; i<S.ticket.length; i++) {
				var t = S.ticket[i];
				t.import = normImport(t.cantidad*t.article.preu);
				...
				var strP = fillSpaceL(formatImport(t.article.preu));
				strP = fillSpaceL(strP,8);
				var strI = fillSpaceL(formatImport(t.import, false));
				strI = fillSpaceL(strI,8);
				
				var lenN = 56-(6+1+0+1+strP.length+1+strI.length);
				var strN = t.article.nom.substring(0,lenN);
				strN = fillSpaceR(strN, lenN);
				var str = strC+" "+strN+" "+strP+" "+strI+"\n";
				Printer.print(str);
			}

			Printer.printLine("=",56);
			Printer.font("A",true,1,2);
			var strT = formatImport(S.total, true);
			var str = "Total : ";
			str = fillSpaceR(str, 42-strT.length)+strT+"\n";
			Printer.print(str);
			Printer.cutPaper();
			Printer.printBuffer();
		}
