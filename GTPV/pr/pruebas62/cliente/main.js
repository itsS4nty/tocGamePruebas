menuPrincipal = function() {
	...
	var opMenu = {
		puerta : {
			text: "puerta",
			run: function() { procesoPuerta.start(depActual); },
			init: function() { procesoPuerta.init(layoutPrincipal.content);	}
		},
		venda : {
			text: "Venda",
			run: function() { procesoVenda.start(depActual);	},
			init: function() { procesoVenda.init(layoutPrincipal.content); }
		},
		editorTeclats : {
			text: "Editor Teclats",
			run: function() { procesoEditorTeclats.start(depActual);	},
			init: function() { procesoEditorTeclats.init(layoutPrincipal.content); }
		},
		cajaFuerte : {
			text: "Caja Fuerte",
			run: function() { procesoCajaFuerte.start(depActual); },
	...
	function dependentesActivesHandler() {
		...
		S = DepActives.getDatosProceso("menuPrincipal", depActual);
		redraw();
	}
...
//		if (posDepAct == -1) { posDepAct=0; codiDependentaActual = null; }
	...
	my.init = function() {
		...
		DepActives.add = function(dep) {
			...
			S = DepActives.getDatosProceso("menuPrincipal", depActual);
			S.menu = "dependenta";
			S.opMenuSel = opMenuInicial["dependenta"];
			redraw();
		}
		...
		S = DepActives.getDatosProceso("menuPrincipal", depActual);
		S.menu = "noDependenta";
		S.opMenuSel = opMenuInicial["noDependenta"];
