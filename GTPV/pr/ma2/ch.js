function ch() {
	...
	function initializeSat(idSat, sat) {
		if (sat != null) satellites.splice(satellites.indexOf(sat), 1);
		var satT = { idSat: idSat, isLocal: (typeof idSat !== "string"), idxSecHS: 1, objs: [], pendingHS: [] };
		satellites.push(satT);
		if ((typeof registrationHandler === "function") && (!registrationHandler(idSat, sat))) {
			sat = null;
			satellites.splice(satellites.indexOf(satT), 1);
		} else {
			sat = satT;
		}
		return sat;
...
*/			} catch(e) {
			...
			if (!isSH && (m.i != null)) { sat = initializeSat(idSat, sat); }
			if (sat != null) {
				if (isSH) {
...
*/			if (applet.isActive()) {
		...
		setTimeout(my.init, 1000, callbackInit, _registrationHandler);
		return false;	
	}
	my.stop = function() {
