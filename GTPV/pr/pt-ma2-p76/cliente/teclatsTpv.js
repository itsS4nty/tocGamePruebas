H.Scripts.addLocalExec("teclatsTpvS", function(window) {
...
//		divAT.height(ambientsScroll.getDiv().height()+divTBE.height());
	...
	var paramsAmbientsScroll = {
		...
		getItem : function(idx) {
			...
			if (amb === D.ambientActual) el.addClass("ui-state-active");
			return el;
		},
	...
	var ambientModel = $("<button>").css({boxSizing: "border-box", margin: "0px"})
	...
	function redrawDivTeclat() {
		...
		for (var i=0; i<buttonsTeclat.length; i++) {
			...
			var article;
			if (data != null) article = Articles.getByCodi(data.codi);
...
//		resizeDivTBE();
	...
	var nivellModel = $("<button>").addClass("ui-state-default");
	...
	var autoCompleteEntrarModel = $("<button>").addClass("ui-state-default");
