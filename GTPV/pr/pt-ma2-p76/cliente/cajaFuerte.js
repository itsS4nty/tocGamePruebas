H.Scripts.addLocalExec("cajaFuerteS", function() {
...
window.AppCajaFuerte = function() {
	...
	var subMenuModel = $("<button>").width("100%").addClass("ui-state-default");
...
//	var S = {};
	...
	var divBT = $("<div>")._100x100().css({ padding: SEP }).addClass("ui-widget-content").appendTo(div0);
	...
	var paramsInfoTickScroll = {
		...
		getItem : function(idx) {
			...
			if (ticket === ticketSelected) el.addClass("ui-state-active");
			return el;
		},
	};
	...
	var butBorrar = $("<button>").text(M("Borrar ticket"))
	...
	function redraw(checkVis) {
		...
		prevLines = [];
		if (ticketSelected) {
			var date = ticketSelected.date;
			var numTick = ticketSelected.numTick;
			var info = ticketSelected.info;
			
			var D = {};
			D.date = date;
			D.numTick = numTick;
			var dep = Dependentes.getByCodi(info.codiDep)
			D.depNom = ((dep != null) && (dep.nom)) ? dep.nom : "";
			D.rowsT = info.rowsT;
			D.total = info.total; 
			D.currencySymbol = "€"; 
	...
	function resizeDivBT() {
		...
		previewTickScroll.setResize();
		previewTickScroll.redraw();
...
//	var S = {};
	...
	var div0 = $("<div>")._100x100();
	var divES = $("<div>")._100x100().css({ padding: SEP }).addClass("ui-widget-content").appendTo(div0);
	...
	var paramsConceptosScroll = {
		...
		getItem : function(idx) {
			...
			if (concepto == conceptoSelected) el.addClass("ui-state-active");
			return el;
		},
	};

	var conceptosModel = $("<div>").css({ boxSizing: "border-box" }).addClass("ui-state-default");
	...
	var paramsAntApuntesScroll = {
		...
		getItem : function(idx) {
			if (idx === -1) return defaultElementConceptosScroll;
			var ap = apuntes[idx];
			var text = fillSpaceL(formatImport(ap.imp, true), 9)+" "+ap.motiu; 
	...
	var butNModel = $("<button>").css({width: "3em", height: "3em", whiteSpace: "pre"}).click(function (e) {
		if (e.button !== 0) return;
...
window.AppCerrarCaja = function() {
	...
	var butCanviCorrecte = $("<button>").css({position: "absolute", width: "100%"})
	                                    .text(M("Canvi Correcte")).appendTo(canvi.getDivButtons());
