...
H.Comm = function () {
	...
	function createSat(idSat, isLocal, tsSat, prevTsInit) {
		...
		sat.getTime = function() {
			var delta = getCurrentTime() - hostTimeInLastSatTime;
			if (delta < 0) delta = 0; // ????
			return lastSatTime+delta;
		}

		sat.diffTime = function(timeSat0) {
			var diff = sat.getTime()-timeSat0;
...
*/		
		...
		function createMessage(sec, data) {		
			...
			if (sec != null) m.sec = sec; // sec siempre es != null ?
			$.extend(m, data);
			return m;
		}
		...
		function responseToS(exchange, sec, data) {
			...
			if (sat.isLocal) { 
				...
				applet.httpEx_response_UTF8(httpEx, 200, objectToUnEval(m), null);
			}
		}
	...
	function processReqBodySat(isLocal, isSH, reqBody, exchange) {
		...
		if (!isSH && (m.i == null)) {
			...
			if (registrationHandler(newSat.interfaceApp, sat ? sat.interfaceApp : null)) {
				if (sat) {
					satellites.splice(satellites.indexOf(sat), 1);
					sat.destroy();
				}
				sat = newSat;
				satellites.push(sat);
			...
			if (!sat || (m.i !== sat.tsInit)) return 401;
		}
...
//			satellites.push({ idSat: idSat, secHS: 1, objs: [], pendingHS: [] });
			...
			var file;
			file = my.findWebPage(handlersWeb, path, idSat, false)
			if (file != null) {
	...
	my.findWebPage = function(handler, path, idSat, byName/*=true -> name = path */) {
		if (handler instanceof Array) {
			for (var i=0; i<handler.length; i++) { 
				var file = my.findWebPage(handler[i], path, idSat, byName);
