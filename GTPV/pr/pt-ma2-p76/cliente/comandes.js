H.Comandes = function() {
	...
	function cobrar(cm) {
		...
		var total = calcTotalComanda(cm);
		var items = cm.items; // no se modifican si esta cerrada pero cm puede que sí
		var codiDep = cm.codiDep;
...
//				var depTemp = dep;
				...
				H.DB.transactionWithErr(db, function (tx) {
					...
					H.Caja.save(tx, function (infoCaja, movimientos) {
						...
						var infoMov = { codiDep: codiDep, rowsT : [], total: total };
						items.forEach(function(t) {
							if (t.n > 0) {
								...
								infoMov.rowsT.push({ n: n, codi: codi, imp: imp }); 
							}	
						});
						infoCaja.sumTick += total;
						infoCaja.clients++;
						H.Caja.insertTicket({ date: sqlDate, numTick: cj.numTick, info: infoMov }, movimientos);
					});
				});
			});
		})
	...
	function getIdItemHost(transIds, idItem) {
					...
					for (var i=0; i<cSat.ord.length; i++) {
						...
						switch (o.cmd) {
							...
							case "mod":
								...
								var idx = findIdxItem(transIds, o.data.id);
								if (idx === -1) { answC.er = ""; break; }
								o.data.id = getIdItemHost(transIds, o.data.id);
...
H.Scripts.addLocalExec("ComandesS", function(window) {
