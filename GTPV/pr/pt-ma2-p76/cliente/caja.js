...
*/
	...
	function testCaja(dbName, callback) {
		...
		db.transaction(function(tx) {
			H.DB.exec(tx, "SELECT caja FROM _g_Caja", [], function(tx,r) {
				...
				try {
					var caja = JSON.parse(r.rows.item(0).caja);
					infoCaja = caja[0];
					movimientos = caja[1];
					...
					function fillInfoT(tx, t) {
						...
						H.DB.exec(tx, stat, [t.date, t.numTick], function(tx, r) {
							...
							for (var ir=0; ir<r.rows.length; ir++) {
								...
								if (ir === 0)  t.info.codiDep = row.codiDep;
	...
	function restoreCaja(callback) {
		lastDBNameCaja = H.GlobalGTPV.get("lastDBNameCaja");
		if (lastDBNameCaja != null) {
			testCaja(lastDBNameCaja[0], function(testOk) {
				if (testOk) {
					lastDBNameCaja = lastDBNameCaja[0];
					callback();
				} else testCaja(lastDBNameCaja[1], function(testOk) {
					lastDBNameCaja = lastDBNameCaja[1];
					H.GlobalGTPV.save("lastDBNameCaja", [lastDBNameCaja, null], function() {
						callback();
					});			
				});
			});
	...
	// y modificar lastDBNameCaja antes de escribir
	var waitFuncs = [];
	...
	my.preSave = function(sqlDate, callback) {
		waitSaveCaja(function() {
			...
			if (lastDBNameCaja != nextDBNameCaja) {
				H.GlobalGTPV.save("lastDBNameCaja", [nextDBNameCaja, lastDBNameCaja], function() {
...
//		if (!H.DB.inTransaction(true, true)) return;
...
//		var sqlDate = H.DB.DateToSql(new Date());
	...
	function insertMovimientoEnDB(tx, tableName, date, dependenta, tipus, imp, motiu, mark) {
		H.DB.sincroInsert(tx, tableName, "[Botiga], [Data], [Dependenta], [Tipus_moviment], [Import], [Motiu], ",
		                               [H.GlobalGTPV.get("Llicencia"), date, dependenta, tipus, imp, motiu], mark);
	}
	...
	my.createSat = function(sat, isAdmin, callback) {
		var obj = sat.createObj("Caja", [createObjSat, isAdmin], [createObjHost, isAdmin], callback, availableCommHandler);
		sats.push(sat);
		objs.push(obj);
				...
				insertMovimiento : function(_ts, tipoMov, mov) {
					...
					switch (tipoMov) {
						case "apuntes":
							...
							var sqlDate = H.DB.DateToSql(new Date());
							openMovimientos(sqlDate, function(tx, h) {
...
H.Scripts.addLocalExec("CajaS", function() {
...
window.newSubAppCanviCaja = function() {
	...
	var div0 = $("<div>")._100x100().addClass("ui-widget-content");
...
H.ConceptosEntrega = function() {
	...
	my.createSat = function(sat, isAdmin, callback) {
		var obj = sat.createObj("ConceptosEntrega", [createObjSat, isAdmin], [createObjHost, isAdmin], callback, availableCommHandler);
		sats.push(sat);
		objs.push(obj);
		obj.data.actualize = { All : true };
		obj.data.comHandler = getComHandler(obj);
