function myScroll(options) {
	...
	function clickArrow() {
		my.scrollTo(currentScroll+($(this).data("inc"))*opt.nElementsScroll);
	}
	function label(t) {
		return opt.labels[t];
	}
	...
	var arrowDiv = [], but = [];
	for (var i=0; i<=1; i++) {
		arrowDiv[i] = $("<div>").css({display: "none"}).css(size, opt.sizeArrow).css(nsize, "100%").appendTo(div);
	}
	var contentDiv = $("<div>").css(opt.contentCSS).addClass(opt.contentClass);
	contentDiv.css({display: disp, boxSizing: "border-box", overflow: "hidden", width: "100%", height: "100%"});
	...
	var divSize = 0;
	isRedrawNeeded = function() {
		return (div[size]() != divSize); 	
	}
	nElements = 0;
	...
	this.newElement = function() {
		...
		el.css(nsize, "100%").css({display : disp, boxSizing : "border-box"}).appendTo(scrollContentDiv);
...
*/		return el;
	...
	this.redraw = function() {
		...
		dispA = (nElements > opt.nElementsScroll) ? disp : "none";
		for (var i=0; i<=1; i++) {
			sa[i]=0;
			if (vAD[i]) {
