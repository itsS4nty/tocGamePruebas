...
function createDefaultTable($Tabla, $arrayCamposIndice) {
	...
	$TablaServer = $Cliente['db'].'['.$Cliente['prefijo'].$Tabla.']';
	$TablaServerSincro = $Cliente['db'].'['.$Cliente['prefijo'].$Tabla.'_SINCRO]';
	
	// verificar tabla server
	$statement = "SELECT TOP 0 * FROM $TablaServer";
	$sql = odbc_prepare($ConnDB, $statement); 
	if (!odbc_execute($sql, array())) { 
		if ($Cliente['db'] != '') { enviarError(330); }
...
////////////////////////
		if ($flag == false) {
			$flag = true;
			for ($ii = 1; $ii<=$numFields; $ii++) {
			    debug_str(odbc_field_name($sql, $ii)." : ".odbc_result($sql, $ii));	
			}
		}
