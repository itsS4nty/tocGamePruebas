var procesoCajaFuerte = function() {
	...
	var opSubMenu = {
		borrarTickets : {
			...
			run: function() { procesoBorrarTickets.start(dep); },
			init: function(callbackInit) { procesoBorrarTickets.init(div02, callbackInit); },
		},
		salidaDinero : {
			text: "Sortida de diners",
			run: function() { procesoESDinero.start("O", dep); },
			init: function(callbackInit) { procesoESDinero.init(div02, callbackInit); },
		},
		entradaDinero : {
			text: "Entrada de diners",
			run: function() { procesoESDinero.start("A", dep); },
			init: function(callbackInit) { procesoESDinero.init(div02, callbackInit); },
		},
		cerrarCaja : {
			text: "Tancar Caixa",
			run: function() { procesoCerrarCaja.start(dep); },
			init: function(callbackInit) { procesoCerrarCaja.init(div02, callbackInit); },
		},
	};
	...
	my.init = function(div00, callbackInit) {
		...
		for (var i in opSubMenu) {
			if (opSubMenu.hasOwnProperty(i)) opSubMenu[i].init(newCallbackInit());
		}
	...
	my.start = function(_dep) {
		divShow(div0);
		dep = _dep;
		S = DepActives.getDatosProceso("cajaFuerte", dep);
		...
		for (var i=0, pos = null; i<subMenus[S.subMenu].length; i++) {
			...
			var el = subMenuModel.clone(true).text(M(opSubMenu[op].text)).data("data", op).appendTo(subMenuScroll.newElement());
...
var procesoBorrarTickets = function() {
	...
	var dialogBorrarTicket = createAlertDialog();
	function setDialogBorrarTicket() {
		dialogBorrarTicket
			.header(M("Borrar?"))
	...
	butBorrar.click(function(e) {
		if (e.button !== 0) return;
		if (S.ticketSelected != null) {
			setDialogBorrarTicket();
			dialogBorrarTicket.text(M("Borrar Ticket n:")+S.ticketSelected.numTick);
	...
	my.start = function(_dep) {
		divShow(div0);
		if (!Caja.get().oberta) {
			DivMensajesCaja.appendTo(div0, M("Caixa\nTancada"));
		} else {
			dep = _dep;
	...
	function redraw() {
		...
		for (var i=0, pos=null; i<tickets.length; i++) {
			var texto = M("Num. Tick : ")+tickets[i].numTick+M("  Data : ")+tickets[i].date;
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			...
			DB.exec(tx, stat, [], function (tx,res) {
				...
				runCallbacksInit();
			}, function (tx,e) { runCallbacksInit(); return false; });
		}, function (e) { runCallbacksInit(); /* DB.errorOpenDB */} );
	}
	var dep; 
