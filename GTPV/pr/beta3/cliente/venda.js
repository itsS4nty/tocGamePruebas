procesoVenda = function() {
	...
	var alertDialog = createAlertDialog();
	function setAlertDialog() {
		alertDialog.header(M("Error")).setButtons([M("Ok")], function(text, i) { S.alertDialog=null; });
	}
	...
	teclatsTpv.selectedArticleHandler = function(data) {
		...
		if (article.esSumable && S.fPeso) {	showAlertDialog(M("Aquest article no es ven a pes")); return; }
		if (!article.esSumable && !S.fPeso) { showAlertDialog(M("Aquest article es ven a pes")); return; }
		...
		if (el == null) {
			...
			el = modelElemTicket.clone(true).insertBefore(terminalTicketScroll)/*appendTo(divTicketScroll)*/;
			setOH(el, hTicketEl);
...
//	var divTotal = $("<div>").css({border: "1px solid black", fontSize: "120%", overflow: "hidden"}).appendTo(divVisorTicketTotal);
	...
	function setButBorrarPesoCobrar() {
