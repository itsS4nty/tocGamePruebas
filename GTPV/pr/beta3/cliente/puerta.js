procesoPuerta = function() {
	...
	var opSubMenu = {
		entradaSalidaDependenta : {
			...
			init: function(callbackInit) { procesoEntradaSalidaDependenta.init(div02, callbackInit); },
		},
		configuracion : {
			text: "Configuración",
			run: function() { div02.children().hide(); },
			init: function(callbackInit) { /*configuracion.init(div02);*/ callbackInit(); }
		},
		impresora : {
			text: "Impresora",
			run: function() { procesoImpresora.start(); },
			init: function(callbackInit) { procesoImpresora.init(div02, callbackInit); }
		},
		salir : {
			text: "Salir",
			run: function() { 			
			...
			init: function(callbackInit) { callbackInit(); }
		}
	};
...
*/	
	...
	function callbackKeyboard1(m) {
		...
		if (m == "enter") {
			...
			DB.preOpenMensual(sqlDate, "V_Horaris_", function(h) {
				...
				DB.transactionWithErr(db, function (tx) {
					...
					DB.sincroInsert(tx, h.tableName, "[Botiga], [Data], [Dependenta], [Operacio], ",
													 ...
													 h.mark);
				}, function() { DB.inTransaction(false); DepActives.add(depSel); }
				);	
			});
		}
	}
	...
	function clickAutoCompleteHandler(e) {
		if (e.button !== 0) return;
...
*/	function plegar(dep) {
		...
		var sqlDate = DB.DateToSql(new Date());
		var h = DB.preOpenMensual(sqlDate, "V_Horaris_", function(h) {;
			var db = DB.open(h.dbName);
			DB.transactionWithErr(db, function (tx) {
				DB.sincroCreate(tx, h.tableName, "[Botiga] float, [Data] datetime, "
												+"[Dependenta] float, [Operacio] nvarchar(25), ");
