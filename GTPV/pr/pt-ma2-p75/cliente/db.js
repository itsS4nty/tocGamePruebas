...
/*	my.getLastMensual = function(callback) {
...
//		var sufix = calcMensualSuffix(date);
	...
	my.setComServer = function(com) { // com:(true,false) -> inicia/finaliza comunicación con servidor
...
*/	
	 ...
	 function errorQuota(e/*, callback*/) {
		var alertErrorDB = createAlertDialog().header("Error").text("Error en base de datos");
/*		alertErrorDB.setButtons(["Ok"], function(text, i) { 
			DB.inTransaction(false); 
			alertErrorDB.getDiv().remove();
			if (typeof errorHandler === "function") {
				errorHandler();
				my.clearErrorHandler();
			}
		});
*/		alertErrorDB.getDiv().appendTo("body");
		alertErrorDB.show();		
	}
	my.transactionWithErr = function(db, transactionCallback, successCallback) {
...
var GlobalGTPV = function() {
	...
	my.set = function(name, value, usePref) {
		...
		db.transaction(function(tx) {
			...
			DB.exec(tx, "INSERT OR REPLACE INTO [gtpv] (name, value) VALUES (?, ?)", [name, value]);
		});
	}
