 var CajaH = function() {
	...
	my.init = function(callback) {
		...
		if (lastDateCaja != null) {
			testCaja(lastDateCaja[0], function(testOk) {
				if (testOk) {
					lastDateCaja = lastDateCaja[0];
					callback();
				} else testCaja(lastDateCaja[1], function(testOk) {
...
*/
		...
		for (var p in CajaActual) {
			if (CajaActual[p] instanceof Array) temp[p] = CajaActual[p].slice(0); // copy array
...
//		var sqlDate = DB.DateToSql(new Date());
	...
	my.createSat = function(type, idSat, callbackInit) {
		...
		newSat.actualize = { All: true , tickets: [], apuntes: [] };
		newSat.comHandler= getComHandler(newSat);
		newSar.type = type;
		satellites.push(newSat);
		CH.createObject("Caja", idSat, objSat, objHost, null, availableCommHandler);
		CH.callFunction("Caja", idSat, "setType", [type]);
	...
	function availableCommHandler(idSat) { // doActualizationSat
		...
		if (sat.actualize.All) {
			data = data || {};
			data.ts = CajaActual.ts;
			data.oberta = CajaActual.oberta;
			if (type === "admin") {
				data.canvi = CajaActual.canvi;
			}
			//sat.waitResponse = true;	
			sat.actualize.All =  false;
			sat.actualize.tickets = [];
			CajaActual.tickets.forEach(function (t) { sat.actualize.tickets.push(t); });
			sat.actualize.apuntes = [];
			CajaActual.apuntes.forEach(function (a) { sat.actualize.apuntes.push(a); });
		} 
		["tickets", "apuntes"].forEach(function (cl) {
			sat.actualize[cl].forEach(function (item) {
				data = data || {};
				data[cl] = data[cl] || [];
				(itemAD = {})[(CajaActual[cl].indexOf(item) !== -1)?"a":"d"] = item;
				data[cl].push(itemAD); 
			});
			sat.actualize[cl] = [];
		});
		...
		if (data != null) {
			CH.callFunction("Caja", sat.idSat, "actualize",	[data], sat.comHandler);
		}		
	}
...
//			doActualizationSat(sat);
	...
	var objHost = {
		...
		deleteApunte : function(apunte) {
		
		}
	}
	
	var objSat = function(host) {
		return createCajaS(host);	
	}
	
	return my;	
}();

Scripts.addLE("CajaS", function(window) {
	window.createCajaS = function(host) {
...
Scripts.add("CajaS", function(window) {
