...
//		getUserData(docOut.documentElement).reloadDB = (getAttribute(nodeIn, "reload") != null);
		if (getAttribute(nodeIn, "reload") != null) DB.reloadDB();
...
*/
		...
		var db = DB.open(name);     // TODO : error db, changeVersion
		...
		if (prefijo != null) {
			...
			if (prevPref != prefijo) {			
				...
				if (prevPref == null) {
					...
					Caja.init(function() { DB.reloadDB(); }); // ???? Canvio prefijo, mejor reinicializar
				}
			}
		}
