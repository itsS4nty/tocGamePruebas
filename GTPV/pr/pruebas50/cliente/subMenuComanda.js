procesoComanda = function() {
	...
	alertDialog.header("Error");
	alertDialog.setButtons(["Ok"], function(text, i) { S.alertDialog=null; alertDialog.hide(); });
	alertDialog.getDiv().appendTo(div0).hide();
	...
	teclatsTpv.getDiv().appendTo(divComanda);
	
	teclatsTpv.changeAmbientHandler = function(data) {
		if (S.estado != "entrar") {
			S.estado = "entrar";
			clearTicket();
...
//		hTeclado : 0.6,
	...
	var divAccions = $("<div>").appendTo(divComanda);
	...
	my.init = function() {
		...
		$(window).resize(function() { 
			...
			if (alertDialog.getDiv().get(0).offsetWidth > 0) alertDialog.redraw(); 
		});
		teclatsTpv.init();
	};
...
*/
		...
		posAbsolute(teclatsTpv.getButtonFindProducte(), 0, h2/3, wControl/2, 2*h2/3); // (0,1)
