function createAlertDialog() {
	...
	my.text = function(text) { divContent.text(text); }
	my.header = function(text) { divHeader.text(text); }	
	my.setButtons = function(butArray, handler) {
		...
		for (var i=0; i<butArray.length; i++) {
			$("<button>").text(butArray[i])
			             ...
			             .appendTo(divButtons).mousedown((function(text, i) { return function() { handler(text, i); } })(butArray[i], i)); 
		}
	}
	my.show = function() {
		div.show();
