...
(function (Server) {
    ...
    */
    ...
    function startUserPassword(user, password) {
        initialClientInfo = {
            user: user,
            password: password,
            producto: "TPV",
    ...
    Server.startUserPassword = startUserPassword;
    ...
    function send(out) {
        window.outmsg = out;
        console.log(out);
        window.outJson = JSON.stringify(out);
        return;
        $.ajax({
            url: urlAjaxServidor,
            type: "POST",
...
*/
    function processResponse(msg) {
        if ($.type(msg) !== "object")
            return "msg"; //??
        if (!Array.isArray(msg.gtpv))
            return "msg.gtpv"; //??
        // check msg.gtpv
        if ($.type(msg.clientInfo) !== "object")
            return "msg.clientInfo"; //??
        var dbs = msg.dbs;
        if ((dbs != null) && (!Array.isArray(dbs)))
            return "dbs"; //??
        /*		if (fInitializingApp) {
        ...
        */
        ...
        function iterateDBs(iDB) {
            ...
            processDB(dbs[iDB], outDbs, function () { iterateDBs(iDB + 1); });
        }
        function end() {
    ...
    function processDB(servDB, outDB, callback) {
        if ($.type(servDB) !== "object") {
            outDB.err = "";
            callback();
            return;
        } //??
        ...
        processSchema(dbName, servDB.schema, function (db, versionChange, err) {
            if (err != null) {
                outDB.err = err;
                callback();
                return;
            ...
            if (servSubtransactions == null)
                servSubtransactions = [];
            if (!Array.isArray(servSubtransactions)) {
                outDB.err = "ooo";
                end();
                return;
            } //?? error
            ...
            db.onerror = function (ev) {
                var req = ev.target;
                outDB.err = req.error.name + ": " + req.error.message;
                end();
            };
            resetSandbox("transaction");
            var iSubtr = 0;
            outDB.transaction = [];
            var abortInfo = { iSubtr: 0, error: null };
            executeIDBTransaction(null);
            function executeIDBTransaction(tr) {
                var outSubtr;
                var pendingSubtr = [];
                if (!tr) {
                    try {
                ...
                tr.oncomplete = function (ev) {
                    for (var i = iSubtr - 1; i >= 0; i--) {
                        if (outDB.transaction[i].aborted == null)
                            outDB.transaction[i].aborted = false;
                    }
                    if (pendingSubtr.length > 0) {
                        executePendingSubtr(db, servSubtransactions[iSubtr], abortOnError, outSubtr, pendingSubtr, abortInfo, function (tr, err) {
                            if (tr) {
                                iSubtr++;
                                if (!(tr instanceof IDBTransaction))
                                    tr = null;
                ...
                tr.onabort = function (ev) {
                    for (var i = iSubtr - 1; i >= 0; i--) {
                        if (outDB.transaction[i].aborted == null)
                            outDB.transaction[i].aborted = true;
                    }
                ...
                tr.onerror = function (ev) {
                    ev.stopPropagation();
                    outDB.err = tr.error.name + ": " + tr.error.message;
                };
                // schema
                if (outDB.schema == null) {
                    outDB.schema = {};
                ...
                function iterateSubtransaction() {
                    if (iSubtr == servSubtransactions.length)
                        return;
                    outSubtr = {};
                    outDB.transaction.push(outSubtr);
                    resetSandbox("subtransaction");
                    abortInfo.iSubtr = iSubtr;
                    processSubtr(tr, servSubtransactions[iSubtr], abortOnError, outSubtr, pendingSubtr, abortInfo, function () {
    ...
    */
    ...
    function processSchema(dbName, servSchema, callback) {
        var fCallback = false;
        function call_callback(db, versionChange, err) {
            if (!fCallback) {
                fCallback = true;
                callback(db, versionChange, err);
            } // por seguridad de no llamar dos veces
        }
        ...
        function onUpgradeSchema(ev) {
            var openReq = ev.target;
            var db = openReq.result;
            var tr = openReq.transaction;
            tr.onabort = function (ev) {
                call_callback(null, false, tr.error.name + ": " + tr.error.message);
            };
            var os;
            try {
                for (var nameObj in servSchema) {
                    ...
                    if (create) {
                        os = db.createObjectStore(nameObj, { keyPath: servObj.keyPath, autoIncrement: servObj.autoIncrement });
                    }
                    else {
        ...
        function onErrorSchema(ev) {
            var openReq = ev.target;
            var err = openReq.error;
            call_callback(null, false, err.name + ": " + err.message);
        }
    ...
    function initSandbox(callback) {
        ...
        function sandboxOnload(ev) {
            console.log("sandboxonload 0");
            var channel = new MessageChannel();
            portSandbox = channel.port1;
            iframe.contentWindow.postMessage({ type: "Function", sharedDataProps: ["golbal", "communication", "message", "transaction", "subtransaction"] }, "*", [channel.port2]);
            portSandbox.onmessage = function (ev) {
                ...
                func.callback.apply(null, data.ret.concat(func.restArgs));
            };
        }
    }
    Server.initSandbox = initSandbox;
    var functionsSandbox = Object.create(null);
    function createFunctionSandbox(name, body, argNames) {
        functionsSandbox[name] = [];
        portSandbox.postMessage({ type: "create", name: name, argNames: argNames, body: body });
        return function () {
            var args = [];
            ...
            portSandbox.postMessage({ type: "execute", name: name, args: args.splice(0, argNames.length), idx: idx });
    ...
    function createModify(script) {
        ...
        if (script == null) {
            ...
            return createFunctionSandbox("modify", script + "; return [" + argNames.join(",") + "];", argNames);
    }
    function createFilter(script) {
        ...
        if (script == null) {
            ...
            return createFunctionSandbox("filter", "return [(" + script + ")];", argNames);
    }
    function resetSandbox(prop) {
        portSandbox.postMessage({ type: "reset", prop: prop });
    }
    function deconstructValue(val, columns, allColumns) {
        if (!columns)
            return val;
    ...
    function cleanOutErrors(outSubtr) {
        if (outSubtr.errors && (outSubtr.errors.length == 0))
            delete outSubtr.errors;
    }
    function processSubtr(tr, servSubtr, abortOnError, outSubtr, pendingSubtr, abortInfo, callback) {
        var savedISubtr = abortInfo.iSubtr;
        function fillAbortInfo(err) {
            if (abortInfo.error != null)
                return;
        ...
        try {
            if ($.type(servSubtr) !== "object")
                throw "servSubtr";
            if ("id" in servSubtr)
                outSubtr.id = servSubtr.id;
            outSubtr.type = servSubtr.type;
            outSubtr.objName = servSubtr.objName;
            var os = tr.objectStore(servSubtr.objName);
            var index;
            if (servSubtr.index != null)
                index = os.index(servSubtr.index);
            ...
            if ((columns != null) && (!Array.isArray(columns)))
                throw "columns";
            if (columns && !columns.every(function (col) { return ((col == null) || (typeof col === "string")); }))
                throw "columns every";
            var allColumns = !!servSubtr.allColumns;
            var valueTypes = servSubtr.valueTypes;
            if ((valueTypes != null) && (typeof valueTypes !== "object"))
                throw "valueTypes";
            if (valueTypes) {
                for (var t in valueTypes) {
                    if ((valueTypes[t] != null) && (!Array.isArray(valueTypes[t])))
                        throw "valueTypes inner";
                }
            }
            ...
            if (lower != null) {
                if (upper != null)
                    keyRange = IDBKeyRange.bound(lower, upper, servSubtr.lowerOpen, servSubtr.upperOpen);
                else
                    keyRange = IDBKeyRange.lowerBound(lower, servSubtr.lowerOpen);
            }
            else if (upper != null) {
                keyRange = IDBKeyRange.upperBound(upper, servSubtr.upperOpen);
            }
            else if (servSubtr.only != null)
                keyRange = IDBKeyRange.only(servSubtr.only);
            ...
            if (typeof servSubtr.abortOnError === "boolean")
                abortOnError = servSubtr.abortOnError;
            ...
            switch (type) {
                ...
                case "add":
                    ...
                    for (var i = 0; i < values.length; i++) {
                        ...
                        if (fPending) {
                            ...
                            var fBreak = (function (val, key, i) {
                                function generateError(ev, tr, err) {
                                    if (abortInfo.error != null)
                                        return;
                                    var outError = {
                                        name: err.name,
                                ...
                                req.onerror = function (ev) {
                                    ev.stopPropagation();
                                    generateError(ev, null, ev.target.error);
                                };
                                return false;
                            })(val, key, i);
                        }
                ...
                case "update":
                    ...
                    if (values && (values.length !== keys.length))
                        throw "values get delete update"; //??
                    if (type === "get") {
                        if (allColumns && (columns == null))
                            columns = [];
                        if (columns != null)
                            outSubtr.columns = columns;
                    }
                    if (!noValues)
                        outSubtr.values = [];
                    if (withKeys)
                        outSubtr.keys = [];
                    if (withPrimaryKeys)
                        outSubtr.primaryKeys = [];
                    outSubtr.errors = [];
                    var fOpenKeyCursor = (noValues && index && (type === "get"));
                    iterateKeys(0);
                    function iterateKeys(iKey) {
                        if (iKey == keys.length) {
                            cleanOutErrors(outSubtr);
                            callback();
                            return;
                        ...
                        try {
                            if (fOpenKeyCursor)
                                req = index.openKeyCursor(keys[iKey], direction);
                            else
                                req = ((index) ? index : os).openCursor(keys[iKey], direction);
                        }
                        catch (e) {
                            if (abortInfo.error != null)
                                return;
            ...
            catch (e) { } // try catch por seguridad, tr no tendria que haberse abortado antes 
            callback();
        }
    }
    function executePendingSubtr(db, servSubtr, abortOnError, outSubtr, pendingSubtr, abortInfo, callback) {
        var savedISubtr = abortInfo.iSubtr;
        function fillAbortInfo(err) {
            if (abortInfo.error != null)
                return;
        ...
        switch (type) {
            ...
            case "update":
                var argsFun = ["val", "key"];
                if (type === "delete")
                    argsFun.push("primaryKey");
                if (type === "update")
                    argsFun.push("primaryKey", "del");
                var funModify = createModify.apply(null, [modify].concat(argsFun));
                var funFilter = createFilter.apply(null, [filter].concat(argsFun));
                for (var i = 0; i < pendingSubtr.length; i++) {
                    var _a = pendingSubtr[i], val = _a[0], key = _a[1], primaryKey = _a[2], del = _a[3];
                    funModify(val, key, primaryKey, del, i, function (val, key, primaryKey, del, i) {
                        funFilter(val, key, primaryKey, del, function (accepted) {
                            if (accepted) {
                            ...
                            if (i == pendingSubtr.length - 1) {
                                ...
                                var iLastReqValid = -1;
                                for (var i = 0; i < pendingSubtr.length; i++) {
                                    if (pendingSubtr[i] == null)
                                        continue;
                                    iLastReqValid = i;
                                    var _a = pendingSubtr[i], val = _a[0], key = _a[1], primaryKey = _a[2], del = _a[3];
                                    (function (val, key, primaryKey, del, i) {
                                        function generateError(ev, tr, err) {
                                            if (abortInfo.error != null)
                                                return;
                                            var outError = {
                                            ...
                                            switch (type) {
                                                ...
                                                case "update":
                                                    if (del)
                                                        ...
                                                        req = os.put(val, inlineKeys ? primaryKey : undefined);
                                                    break;
                                            }
                                            req = (type === "put") ? os.put(val, key) : os.add(val, key);
                                        }
            ...
            case "get":
                ...
                for (var i = 0; i < pendingSubtr.length; i++) {
                    var _b = pendingSubtr[i], val = _b[0], key = _b[1], primaryKey = _b[2];
                    funModify(val, key, primaryKey, i, function (val, key, primaryKey, i) {
                        funFilter(val, key, primaryKey, function (accepted) {
                            if (accepted) {
                                if (outSubtr.values)
                                    outSubtr.values.push(deconstructValue(val, columns, allColumns));
                                if (outSubtr.keys)
                            ...
                            if (i == pendingSubtr.length - 1) {
                                cleanOutErrors(outSubtr);
                                end(true, null); // no hay transaction pero operación correcta, ya se creara
                            }
                        });
                    });
                }
