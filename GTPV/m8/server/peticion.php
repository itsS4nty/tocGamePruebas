...
//	if ($initNode->tagName != 'init') enviarError(89);
	...
	$attrNames = array('user', 'password', 'producto', 'version', 'prefijo', 'idCom', 'ipLan');
	for ($i=0; $i<count($attrNames); $i++) {
		$init[$attrNames[$i]] = getAttribute($initNode, $attrNames[$i]); 	
	}
	$init['ipWan'] = $_SERVER['REMOTE_ADDR'];
			...
			if ($fInsert) {
				...
				$exec = odbc_execute($sql, 
					array($init['idCom'], $init['user'], $init['password'], $init['producto'], $init['version'], 
						  $init['user'].'_', $init['user'].'_', NULL, 1, $init['ipWan'], $init['ipLan']));
	...
	if ($init['idCom'] !== NULL) {
		...
		if (odbc_num_rows($sql) !== 1) {
			...
			$sql = odbc_prepare($ConnDB,
			    "UPDATE G_CLIENTES SET [idCom]=NULL WHERE [idCom]=? ");
			$exec = odbc_execute($sql, array($init['idCom']));
			$init['idCom'] = NULL;
		} else {
			$cliente = array();
			...
			if ($init['ipLan'] === NULL) $init['ipLan'] = $cliente['ipLan'];
			if (($init['ipLan'] !== $cliente['ipLan']) || ($init['ipWan'] !== $cliente['ipWan'])) {
				$sql = odbc_prepare($ConnDB,
					"UPDATE G_CLIENTES SET [ipWan]=?, [ipLan]=? WHERE [idCom]=?");
				$exec = odbc_execute($sql, array($init['ipWan'], $init['ipLan'], $init['idCom']));
				if (!$exec) enviarODBCError("405# Update G_CLIENTES");
			}
			return true;
		}
	}
