...
//					curTsValid : c.curTsValid,
	...
	function genError(c, codError, o) {
		...
		var errorHandler = o.C.getErrorHandler();
		if (errorHandler) 
			window.setTimeout(errorHandler, 0, o.C, codError, o, c, function() { resumeError(c); });	
	}
		...
		for (var i=0; i<resps.length; i++) {
			...
			var procResp = function (idComanda, a) { // a: answer from Host
				return function() {
					...
					if (a.items == null) { // samever en host
...
window.agruparComanda = function(opt, C_o_Items) {
	...
	function _getItems() {
		if (C_getItems == C.getItems()) return;
		C_getItems = C.getItems();
		
		itemsGr_All = [];
		itemsNotInGr = [];
		var items = C_getItems.slice(0); // copy array
	...
	my.getItems = function(reload) {
		if (reload) C_getItems = null;
		_getItems();
		return itemsGr;
	}
	...
	my.equal = function(gr1, gr2) {
		...
		var ids1 = gr1.items.map(function(item) { return C.getIdItem(item); });   
		var ids2 = gr2.items.map(function(item) { return C.getIdItem(item); });   
