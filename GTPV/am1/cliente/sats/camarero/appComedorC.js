H.Scripts.add("sats/camarero/appComedorC", "C", function() {

window.createAppComedor = function(main, controlMain) {
	var my = {};
	...
	function redraw() {
		...
		var divMs = createDiv().appendTo(dC);
		divMs.oHeight(H-title.oHeight()).css({border: "2px solid red"});
		var dM = createDivAbs();
...
//			dC.oHeight(H);
			dC.appendTo(divCs);
			$("<div>").css({whiteSpace: "pre", textAlign: "center", background: "red"})
			          .text(c.name).appendTo(dC);
			var divMs = $("<div>").css({position: "relative"})
	...
	var paramsScroll = { dir: 'x' };
	var scroll = scrollTPages(divCs, touchH, controlTouchScroll, paramsScroll);
