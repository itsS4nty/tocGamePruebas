...
<body>
    ...
    <?php
		$Imprimir = $_GET[Impresion];	// Recoge lo que se va a imprimir
		$puerto = $_GET[puerto];		// Recoge el puerto escogido

		$b=false;
		if($Imprimir == ""){	// Si no se ha escrito nada
			$b = false;
		}else{
			$b = true;
		}
		if($b==true){	// Si se ha escrito algo
			if($puerto == ""){
				// Se ejecutará cuando no se haya escogido ningun puerto
				$b = false;
    ...
    <div id="applet">
        ...
        No puedes imprimir...
            <param name="Imprimir" value='<?php echo $Imprimir; ?>'/>	<!-- envío el texto a imprimir -->
            <param name="puerto" value='<?php echo $puerto; ?>'/>		<!-- envío el puerto por el cual imprimir -->
        </applet>
    </div>
    <?php }	?>
