...
*/		remove_n0: true	
	...
	function cobrar2(cc, c /*sqlDate, total, items, codiDep, tPrint*/) {	
...
//				var depTemp = dep;
				...
				H.DB.transactionWithErr(db, function (tx) {
					...
					H.Caja.save(tx, function (infoCaja, movimientos) {
						infoCaja.numTick++;
						var dep = H.Dependentes.getByCodi(cc.codiDep)
						...
						var itemsAC = agruparComanda(optAgruparComanda_Cobrar, cc.items);
						itemsAC.forEach(function(t) {
							if (t.el.n > 0) {
								...
								H.DB.sincroInsert(tx, h.tableName, 
									...
									[H.ConfigGTPV.get("Llicencia"), cc.sqlDate, cc.codiDep, infoCaja.numTick,
...
H.Scripts.add("ComandesS", "L2", function() {
...
//					curTsValid : c.curTsValid,
	...
	function genError(c, codError, o) {
		...
		if (typeof controlHandler === "function") 
			window.setTimeout(controlHandler, 0, o.C, codError, o, c, function() { resumeError(c); });	
	}
	...
	function callbackSendOrdersToHost(resps, er) {
		...
		if (nextOrdersSendPending) {
			...
			timeoutIdCom = window.setTimeout(sendOrdersToHost, 0);
		}
	}
...
window.optAgruparComanda_Default = {
	group: function(grItemsDest, item) { // return idx donde insert item en grItemsDest, -1 sino se puede agrupar
		return -1;
	},
	append: function(grItemsDest, item) { // ret si item se puede añadir a algun grItemsDest sin crear ningun item nuevo solo incr.
		return false;
	},
	increment: function(grItemsDest, inc) { // ret array de increments correspondientes a cada item de grItemsDest
		return [inc];                       // incrementos en array == null => no incremento
	},
...
window.agruparComanda = function(opt, C_o_Items) {
	...
	if (C_o_Items instanceof Array) { // solo agrupar Items
		my.setC({
			getItems: function() { return C_o_Items; }
		});
		return my.getItems();
	} else if (C_o_Items != null) my.setC(C_o_Items);

	
	my.append = function(item) {
		my.getItems();
		for (var i=0; i<itemsGr_All.length; i++) {
			if (opt.append(itemsGr_All[i].items, item)) {
