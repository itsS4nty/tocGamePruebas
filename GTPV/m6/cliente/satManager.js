H.AppSatManager = function() {
	...
	var paramsLocalAddressesScroll = {
		...
		getItem : function(idx) {
			...
			var el = localAddressesModel.clone(true).text(localAddresses[idx].match(/[^\/]*/)[0]);
			return el;
		},
	};

	var localAddressesModel = $("<div>").addClass("g-state-default");
	...
	var satellitesModel = $("<div>").addClass("g-state-default");
	...
	my.start = function(_mp) {
		...
		function refresh() {
			if (!fAppletActive) {
				var temp = H.Comm.getSiteLocalAddresses();
				if (temp != null) {
					localAddresses = temp;
					fAppletActive = true;
				}
