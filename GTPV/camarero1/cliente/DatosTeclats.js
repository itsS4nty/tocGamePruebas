H.Teclats = function() {
	...
	function obtenerDB() {
		...
		db.transaction(function (tx) {
			...
			var params = [];
			if (H.ConfigGTPV.get("Llicencia") != null) {
				stat += " AND (([Llicencia] = ?) OR ([Llicencia] IS NULL))";
				params.push(H.ConfigGTPV.get("Llicencia"));
			}
		...
		function procesarDatos() {
			...
			datosTeclats.forEach(function(amb) {
				amb.buttons.forEach(function(but) {
					delete but.pos;
					delete but.ambient;
				});
			});
			...
			if (H.DebugDatos) {
				H.DebugDatos.datosTeclats = datosTeclats;
			}
	...
	my.createSat = function(sat, callback) {
		var obj = sat.createObj("Teclats", createObjSat, createObjHost, null, availableCommHandler);
		sats.push(sat);
		objs.push(obj);
		...
		return $.extend(Object.create(null), {
					...
					H.DB.transactionWithErr(db, function(tx) {
						...
						buttons.forEach(function(but) { 
							H.DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
								{ ambient: newAmbient, pos: but.pos, llicencia: H.ConfigGTPV.get("Llicencia") },
			...
			delAmbient : function(ambient, versionSat) {
				...
				H.DB.preOpenPrincipal("TeclatsTpv", function(h) {
					...
					H.DB.transactionWithErr(db, function(tx) {
						H.DB.sincroDelete_UD(h.dbName, tx, h.tableName, 
							{ ambient: ambient, llicencia: H.ConfigGTPV.get("Llicencia") }, h.mark);
					});
				});
				return version;
			},
	...
	//			but.idxArticle = findIdxArticleByCodi(but.codi);
				t.buttons[but.pos] = { codi: but.codi, color: but.color};
				datosTeclats = datosTeclats.sort(getToLowerSortFunction("ambient"));
				...
				H.DB.preOpenPrincipal("TeclatsTpv", function(h) {
					...
					H.DB.transactionWithErr(db, function(tx) {
						H.DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
							{ ambient: but.ambient, pos: but.pos, llicencia: H.ConfigGTPV.get("Llicencia") },
							{ article: but.codi, color: but.color, Dependenta: codiDep, 
							  Data: H.DB.DateToSql() }, h.mark);
					});
				});
			...
			delArticle : function(but, versionSat) {
				if ((but.pos == null) || (but.codi == null)) return version;
				...
				H.DB.preOpenPrincipal("TeclatsTpv", function(h) {
					...
					H.DB.transactionWithErr(db, function(tx) {
						H.DB.sincroDelete_UD(h.dbName, tx, h.tableName, 
							{ ambient: but.ambient, pos: but.pos, llicencia: H.ConfigGTPV.get("Llicencia") }, h.mark);
					});
				});
				return version;
			},
	...
	//			if (but2.codi != null) but2.idxArticle = findIdxArticleByCodi(but2.codi);
				...
				if (t1 != null) { t1.buttons[but1.pos] = { codi: but2.codi, color: but2.color }; }
				if (t2 != null) { t2.buttons[but2.pos] = { codi: but1.codi, color: but1.color }; }
				...
				H.DB.preOpenPrincipal("TeclatsTpv", function(h) {
					...
					H.DB.transactionWithErr(db, function(tx) {
						if (but1.pos != null) {
							H.DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
								{ ambient: but1.ambient, pos: but1.pos, llicencia: H.ConfigGTPV.get("Llicencia") },
								{ article: but2.codi, color: but2.color, Dependenta: codiDep, 
								  Data: H.DB.DateToSql() }, h.mark);
						}
						if (but2.pos != null) {
							H.DB.sincroUpdate_UD(h.dbName, tx, h.tableName, 
								{ ambient: but2.ambient, pos: but2.pos, llicencia: H.ConfigGTPV.get("Llicencia") },
	...
	//			but.color = color;
				if (t.buttons[but.pos] != null) t.buttons[but.pos] = { codi: null };
				t.buttons[but.pos].color = color;
...
H.Scripts.add("DatosTeclatsS", "LTC", function() {
