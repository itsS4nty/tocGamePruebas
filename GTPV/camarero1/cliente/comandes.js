...
*/
	...
	my.createSat = function(sat, callback) {
		var obj = sat.createObj("Comandes", createObjSat, createObjHost, null, availableCommHandler);
		sats.push(sat);
		objs.push(obj);
	...
	function availableCommHandler(objSat) { // doActualizationSat
		...
		if ((qs.length > 0) || (objSat.data.callbackCreateAct)) { 	
		...
		return $.extend(Object.create(null), {
			orders : function(comdSat) {
				...
				for (var csIdx=0; csIdx<comdSat.length; csIdx++) {
					...
					function checkOpenComanda() { // check open comanda
						if ((isOpenComanda()) && (tsSat == c.ts)) return true;
						answC.er = "H not open";
						return false;
					}
					function checkClosedComanda() { // check closed comanda
...
H.Scripts.add("ComandesS", "LTC", function() {
