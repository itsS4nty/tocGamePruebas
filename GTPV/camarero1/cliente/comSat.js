...
H.Comm = function () {
	...
	function createSat(idSat, isLocal, tsSat, prevTsInit) {
		...
		var lastTsComHS = getCurrentTime();
	...
	function processReqBodySat(isLocal, isSH, reqBody, exchange) {
		...
		if (!isSH && (m.i == null)) {
			...
			if (!sat || (m.i !== sat.tsInit)) {
				var res = { r: true, ts: getCurrentTime(), er: (sat ? "conflict" : "reinit") };
				sendMessageToExchange(res, exchange, isLocal);
				return 0;
			}
		}
		...
		if ((method === "POST") && ((path === "/sh/") || (path === "/hs/"))) {
			...
			function sendResponseFile(file) {
				if (file != null) {
					...
					if (file.binary) {
						applet.httpEx_response_String_u0100(httpEx, 200, body, null);
					} else{
						applet.httpEx_response_String(httpEx, 200, body, file.utf8, null);
					} 	
...
H.Scripts.add("ComSatS", "LTC", function() {
