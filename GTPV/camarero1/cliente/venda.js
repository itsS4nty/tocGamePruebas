H.Scripts.add("VendaS", "LT", function() {
...
window.AppVenda = function() {
	...
	function controlHandler(e) {
		...
		switch ($(this).data("tipo")) {
			...
			case "down" :
				...
				break;	
			case "+" :
				var gr = S.AC.getGr(S.T.itemSelected);
				if (gr != null) {
					S.AC.increment(gr, 1);
					changeComanda();
				}
	...
	function cobrarHandler(e) {
		...
		switch (S.estado) {
			case "entrar":
//				abrirCajon();
				if (S.AC.getGrs().length === 0) return;
				calcularTotalTicket();
...
//		hTeclado : 0.6,
	...
	var divAccions = createDivRel().appendTo(divVenda); // overflow : hidden -> visor tamaño fijo
	var divControl = createDiv().appendTo(divAccions);
	var divVisorTicket = createDiv().css({fontFamily: "monospace", whiteSpace: "pre"}).appendTo(divAccions);
	var divTecladoCantidad = createDivRel().appendTo(divAccions);
	divAccions.children().css({display: "inline-block", verticalAlign: "middle"});
	
	// divControl
	var divBuscarArticle = createDiv().appendTo(divControl);
	teclatsTpv.getButtonBuscarArticle().css({ height: "100%" , cssFloat: "right"}).appendTo(divBuscarArticle);
	var divOtrosButtons = createDivRel().appendTo(divControl);
	...
	// divVisorTicketTotal
	var divVisor = createDiv().css({border: "1px solid black", fontSize: "200%", fontWeight: "bold" }) 
	                          .appendTo(divVisorTicket);
	var divTotal = createDiv().css({border: "1px solid black", textAlign: "left", display: "inline-block", 
	                                padding: "0 0.5em", verticalAlign: "middle"})
	                         .appendTo(divVisor); 
	var divCantidad = createDiv().css({border: "1px solid black", textAlign: "right", display: "inline-block", 
	                                   padding: "0 0.5em", verticalAlign: "middle", backgroundColor: "yellow"})
	                            .appendTo(divVisor);  
	var divContTicket = createDiv().css({backgroundColor: "white"}).appendTo(divVisorTicket);
	var divTicketScroll = createDivRel().css({fontWeight: "bold" }).appendTo(divContTicket);
	var terminalTicketElement = createDiv().css({backgroundColor: "black"}).height(1).appendTo(divTicketScroll);
...
//	$("div", divVenda).css({boxSizing: "border-box"/*, visibility: "hidden"*/});
	...
	var elemTicketModel =  createDiv().css({borderTop: "1px dashed black", width: "100%"})
	                                  .mousedown(elemTicketSelectHandler);
	var hContTicket = 0;
...
//		hAT = divT.height();
	...
	function calcularTotalTicket() {
		S.total = 0;
		S.AC.getGrs().forEach(function(item) {
	...
	function ticketScroll() {
		...
		var idx = -1;
		var nGrs = S.AC.getGrs().length;
		var top;
		if (nGrs == 0) {
			S.TScroll.itemRef = null;
			top = 0;
		} else {
			idx = S.AC.getPosGr(S.TScroll.itemRef);
			if (idx === -1) {
				idx = nItems-1;
		...
		if (idx !== -1)	S.TScroll.itemRef = S.AC.getGrs()[idx];
	...
	function scrollTToVisible(item) {
		var	idx1 = S.AC.getPosGr(S.TScroll.itemRef);
		var idx2 = S.AC.getPosGr(item);
	...
	var optAgruparComanda_Venda = {
		group: function(gr_items, item) {
			...
			if (gr_items[0].codi === item.codi) return 0;
			else return -1;
		},
	...
	function changeComanda(C, autoChange) {
		...
		if (C == null) C = S.C; // por autollamada de esta App
		if (C === S.C) {
			S.AC.reload();
			...
			if (S.C.isOpen() && (S.estado === "entrar")) {
				var itemSelected = S.AC.getGr(S.T.itemSelected);
				var elSelected = null;
				var itemRecentIns = S.AC.getGr(S.T.recentIns);
				S.T.recentIns = null;
