...
*/
	...
	function runCloseHandlers() {
		closeHandlers.forEach(function(h) { h(); });
	}
	my.addCloseHandler = function(closeHandler) {
	...
	function availableCommHandler(objSat) { // doActualizationSat
		...
		if (data.actualize.All) {
			objSat.call("actualize", [ts, infoCajaSat, data.isAdmin ? movimientos : null], data.comHandler);
			data.actualize = {};
			return;
		}	
		...
		if (isAdmin) {
			$.extend(objHost, {
				...
				abrir : function(_ts, canvi, codiDep) {
					...
					openMovimientosDB(sqlDate, function(tx, h) {
						my.save(tx, function(infoCaja, movimientos) {
							...
							infoCaja.clients = 0;
							for (var p in movimientos) { // pasado por referenecia
				...
				cerrar : function(_ts, canvi, codiDep) {
					var state = getStateWhenModifyDB();
					if ((!state.oberta) || (state.ts != _ts)) return false;
					nextStates.unshift({ts: Date.now(), oberta: false });
...
H.Scripts.add("CajaS", "LTC", function() {
