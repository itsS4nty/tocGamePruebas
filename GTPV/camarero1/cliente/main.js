	...
	function satelliteRegistrationHandler(sat, oldSat) {
		...
		loadDataS(sat, function() {
			...
			if (configSat.typeApp == "camarero") {
				...
				var menuData;
				if (sat.isLocal()) {
					menuType = "menuPrincipal.menusSatelliteLocal";
				} else menuType = "menuPrincipal.menusSatelliteExtern";	
				
				sat.sendScript("\
					menuPrincipal.menus = \
						menuPrincipal.createOptions("+menuType+", menuPrincipal.opcionesMenu);\
					menuPrincipal.init();\
					menuPrincipal.start();");	
			}
		});
