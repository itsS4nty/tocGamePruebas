...
H.AppImpresora = function() {
	...
	function showAppletSerialError() {
		alertAppletNotLoaded.header(M("Error")).text(M("rxtx no activo"))
								               .setButtons(["Ok"], function(text, i) { mp.finApp(); }).show();
	}
	var selectedP;
	...
	function rememberCloseTestPrinter() {
		if (!div0.isVisible()) closeTestPrinter();
			...
			if (timeoutIdCloseTestP == null) 
				timeoutIdCloseTestP = window.setTimeout(rememberCloseTestPrinter, 1000);
		}
	}
	...
	function selectCurrentPortHandler(e) {
		if (e.button !== 0) return;
		if (selectedP == null) return;
		if (H.curPrinter.getName() != selectedP) {
			...
			H.ConfigGTPV.set("serialPort", selectedP, false);
		}
		mp.finApp();
	}
	
	var divP = $("<div>")._100x100().addClass("g-widget-content").appendTo(div0);
	...
	var serialModel = gButton().addClass("g-state-default");
	...
	var divButtons = $("<div>").css({ position: "absolute", right : "0px", bottom : "0px", padding: SEP, width: "40%"})
	                           .appendTo(divP);
	var butTest = gButton().css({width: "100%"}).appendTo(divButtons).click(testSerialHandler);
	var butSel = gButton().css({width: "100%"}).appendTo(divButtons).click(selectCurrentPortHandler);
			
	my.init = function(div00) {
		div0.appendTo(div00).hide();
		var port = H.ConfigGTPV.get("serialPort", false);	
...
H.Scripts.add("printerS", "LT", function() {
