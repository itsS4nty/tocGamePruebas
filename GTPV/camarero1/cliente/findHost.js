	...
	function findHost() {
		...
		function connectToHost(user, ipHosts) {
			...
			function tryHost(ip, callback, fFirst) {
				...
				return $.ajax({
					url: "http://"+ip+"/_detect/jsonp.php?callback=?",
					dataType: "jsonp",
					timeout: (fFirst ? 5000: 20000),
					success: function(data, textStatus, jqXHR) {
						var hostInfo = ((user == null) || (user === data.user)) ? data : null; 
//						var itemH = findInHostIdent(user);
						callback(jqXHR, hostInfo, ip);	
					},
			...
			function callbackNextTryHost(jqXHR, hostInfo, ip) {
			...
			button("Cancelar y registrar manualmente").appendTo("body").width("50%").css("margin", "10px auto").click(function(e) {
		...
		function selectIdSat(hostInfo, ip) {
			function entryToHostApp(idSat) {
				setCookie("type", "sat");
				setCookie("user", hostInfo.user);
				setCookie("ipLanHost", hostInfo.ipLanHost);
				setCookie("id", idSat);

				$("body").empty();
				$("body").css({position:"absolute", margin:"0", border:"0", padding:"0", width:"100%", height:"100%"});
				var iframe = $("<iframe>");
				iframe.attr("src", "http://"+ip+"/entryS.php?id="+idSat);
			...
			if (idSat != null) entryToHostApp(idSat);
				...
				div1.empty();
				for (var i=0; i<hostInfo.idsSatAllowed.length; i++)
					button("Asignar "+hostInfo.idsSatAllowed[i]).click(getHandler(hostInfo.idsSatAllowed[i])).appendTo(div1);
				for (var i=0; i<hostInfo.idsSat.length; i++)
					button("Reasignar "+hostInfo.idsSat[i]).click(getHandler(hostInfo.idsSat[i])).appendTo(div1);
			}
		}
	...
	if (window.H) {
		H.Scripts.add("findHostS", "0", findHost);
	} else {
		findHost();	
	}
})();
