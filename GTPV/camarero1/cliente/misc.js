H.Scripts.addLocalExec("miscS", "LTC", function() {

var WW=window;

	// firefox no soporta box-sizing
	// chrome no soporta transtion-...
(function() {
	...
	props.forEach(function(prop) {
		...
		if (prop == "transition") WW.transitionSupported = (supportedProp!=null);
...
(function() {
	// linear-gradient en background y backgroundImage (no esta en chrome hasta ver 26.0)
	// pasar a -webkit-liner-gradient, 
	// to top left pasar a bottom right
	var prefs = ["moz", "webkit", "o", "ms"];
	for (var i=0; i<prefs.length; i++) {
		var d = document.createElement( "div" );
		var bg = d.style["backgroundImage"];
		d.style["backgroundImage"] = "-"+prefs[i]+"-linear-gradient(left, black, white)";
		if (bg != d.style["backgroundImage"]) break;
	}
	var pref = (i<prefs.length) ? "-"+prefs[i]+"-" : "";
	WW.linearGradient = function(args) {
		if (pref !== "") {
			var r = /\s*to\s*(top|bottom|left|right)\s*(top|bottom|left|right)?/;
			args = args.replace(r, function(match, p1, p2) {
				var sides = ["top", "right", "bottom", "left"];
				p1 = sides[(sides.indexOf(p1)+2)%4];
				p2 = (p2 == null) ? "" : " "+sides[(sides.indexOf(p2)+2)%4];
				return p1+p2;
			});
		}
		return pref+"linear-gradient("+args+")";	
	}	
})();

WW.transitionEndEventNames = "transitionend webkitTransitionEnd";
...
*/
WW.createDiv = function(fInline) { 
	var div = $("<div>").css({boxSizing: "border-box", overflow: "hidden"}); 
	if (fInline) div.css({display: "inline-block", verticalAlign: "top"});
	return div;
}
WW.createDivAbs = function(fInline) { 
	return createDiv(fInline).css({position: "absolute"}); 
}
WW.createDivRel = function(fInline) { 
	return createDiv(fInline).css({position: "relative"}); 
}
// WW.createDivInline = function() { return createDiv().css({display: "inline-block", verticalAlign: "top"}); }
WW.createDivTable = function() { 
	return createDiv().css({display: "table", width: "100%", height: "100%"}); 
}
WW.createVerticalAligner = function(content, fWidth100x100, textAlign) {
	if (textAlign == null) textAlign = "center";
	var t = createDiv().css({display: "table", height: "100%", textAlign: textAlign});
	if (fWidth100x100) t.css({width: "100%"});
	content.css({display: "table-cell", verticalAlign: "middle"}).appendTo(t);
	return t;
}

WW.colorToHsl = function(color) {
...
// devuelve [firstParent, child, child, ..., source] o null
WW.getDOMParents = function(source, firstParent) {
	if ((source == null) || (firstParent == null)) return null;
	source = ($(source))[0];
	firstParent = ($(firstParent))[0];
	
	var parents = [];

	while (source != null){
		parents.unshift(source);
		if (source == firstParent) return parents;
...
WW.getCookie = function(name) {
	var aC=document.cookie.split(";");
	for (var i=0; i<aC.length; i++) {
		var p = aC[i].indexOf("=");
		if (p == -1) continue;
		var x = aC[i].substr(0,p).replace(/^\s+/g,'');
		if (x === name) {
			var y = aC[i].substr(p+1).replace(/^\s+/g,'');
			return unescape(y);
		}
	}
	return null;
...
WW.setCookie = function(name, value) {
	document.cookie= name+"="+escape(value)+";path=/;max-age="+10*365*24*60*60;
}

WW.deleteCookie = function(name) {
	document.cookie= name+"=;path=/;max-age=0";
}

// Para browsers no compatibles
