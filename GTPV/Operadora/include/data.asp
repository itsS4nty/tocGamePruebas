	...
	if len(session ( "Usuari_Recurso_id" ))  > 0 then 
		...
		if  not rs.eof then
			...
			if len(Filtre)> 0 then
				FiltreWhere = " Where Viatge in ('" & join(split(Filtre,", "),"','") & "')  "
				FiltreAnd = " And Viatge in ('" & join(split(Filtre,", "),"','") & "') "
			end if
		end if
	end if
...
function clientesHoy()
	sql = "select distinct s.client as cod,c.nom from " & tabla & " s left join clients c on s.client=c.codi " 
	sql = sql & " group by s.client,c.nom "
	sql = sql & FiltreWhere
...
'viaje +
	...
	sql = sql & "where ((v.minutinici>" & inici & ")or(v.minutinici=" & inici & " and s.viatge>'" & viaje & "')) "
	sql = sql & FiltreAnd
	sql = sql & " order by v.minutinici,s.viatge"
	set rs=rec(sql)
	if not rs.eof then
		viajeMas = viajes & rs("viatge")
	else
		sql = "select top 1 s.viatge,v.minutinici from " & tabla & " s join viatges v on s.Viatge=v.nom "
		sql = sql & FiltreWhere
		sql = sql & " order by v.minutinici,s.viatge "
		set rs=rec(sql)
...
sql = sql & "where ((v.minutinici<" & inici & ")or(v.minutinici=" & inici & " and s.viatge<'" & viaje & "')) "
sql = sql & FiltreAnd
sql = sql & " order by v.minutinici desc,s.viatge desc"
set rs=rec(sql)
