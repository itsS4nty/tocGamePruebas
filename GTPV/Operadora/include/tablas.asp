...
function tablaVentas ( byval D )
	Bak = ""
	if Session("Data_BKP_Mensuals")<>"" then
		if cdate(Session("Data_BKP_Mensuals")) > cdate(D) then Bak = "_bak"
	end if
	
    if Session("Usuari_NivellSeguretat") = 5 then
		tablaVentas = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Venut_Previsio_" & year(D) & "-" & right("00" & month(D),2) & "]"
    else
		tablaVentas = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Venut_" & year(D) & "-" & right("00" & month(D),2) & "]"
	end if
	if not exists (tablaVentas) then
		sqlCTV = "CREATE TABLE " & tablaVentas & " ( "
		sqlCTV = sqlCTV & "[Botiga] [float] NULL , "
		sqlCTV = sqlCTV & "[Data] [datetime] NULL , "
		sqlCTV = sqlCTV & "[Dependenta] [float] NULL , "
		sqlCTV = sqlCTV & "[Num_tick] [float] NULL , "
		sqlCTV = sqlCTV & "[Estat] [nvarchar] (25) NULL , "
		sqlCTV = sqlCTV & "[Plu] [float] NULL , "
		sqlCTV = sqlCTV & "[Quantitat] [float] NULL , "
		sqlCTV = sqlCTV & "[Import] [float] NULL , "
		sqlCTV = sqlCTV & "[Tipus_venta] [nvarchar] (25) NULL , "
		sqlCTV = sqlCTV & "[FormaMarcar] [nvarchar] (255) NULL DEFAULT ('') , "
		sqlCTV = sqlCTV & "[Otros] [nvarchar] (255) NULL DEFAULT ('') "
		sqlCTV = sqlCTV & ") ON [PRIMARY]"
		rec sqlCTV
	end if
end function
...
function tablaAnulats ( byval D )
	...
	tablaAnulats = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Anulats_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaAnulats) then
		sqlCTA = "CREATE TABLE " & tablaAnulats & " ( "
		sqlCTA = sqlCTA & "[Botiga] [float] NULL, "
		sqlCTA = sqlCTA & "[Data] [datetime] NULL, "
		sqlCTA = sqlCTA & "[Dependenta] [float] NULL, "
		sqlCTA = sqlCTA & "[Num_tick] [float] NULL, "
		sqlCTA = sqlCTA & "[Estat] [nvarchar] (25) NULL, "
		sqlCTA = sqlCTA & "[Plu] [float] NULL, "
		sqlCTA = sqlCTA & "[Quantitat] [float] NULL, "
		sqlCTA = sqlCTA & "[Import] [float] NULL, "
		sqlCTA = sqlCTA & "[Tipus_venta] [nvarchar] (25) NULL, "
		sqlCTA = sqlCTA & "[FormaMarcar] [nvarchar] (255) NULL, "
		sqlCTA = sqlCTA & "[Otros] [nvarchar] (255) NULL "
		sqlCTA = sqlCTA & ") ON [PRIMARY]"
		rec sqlCTA
	end if
end function
...
function tablaAlertes ( byval D )
	...
	tablaAlertes = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Alertes_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaAlertes) then
		sqlCTA = "CREATE TABLE " & tablaAlertes & " ( "
		sqlCTA = sqlCTA & "[Id] [uniqueidentifier] NULL CONSTRAINT [DF__V_Alertes_" & year(D) & "_" & right("00" & month(D),2) & "_ID] DEFAULT (newid()), "
		sqlCTA = sqlCTA & "[Data] [datetime] NULL, "
		sqlCTA = sqlCTA & "[Texte] [nvarchar] (255) NULL, "
		sqlCTA = sqlCTA & "[Revisada] [int] NULL CONSTRAINT [DF__V_Alertes__Revis__01DB011D] DEFAULT (0), "
		sqlCTA = sqlCTA & "[Param1] [nvarchar] (255) NULL, "
		sqlCTA = sqlCTA & "[Param2] [nvarchar] (255) NULL, "
		sqlCTA = sqlCTA & "[Param3] [nvarchar] (255) NULL, "
		sqlCTA = sqlCTA & "[Param4] [nvarchar] (255) NULL "
		sqlCTA = sqlCTA & ") ON [PRIMARY]"
		rec sqlCTA
	end if
end function
...
function tablaHoraris ( byval D )
	...
	tablaHoraris = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Horaris_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaHoraris) then
		sqlCTH = "CREATE TABLE " & tablaHoraris & " ( "
		sqlCTH = sqlCTH & "[Botiga] [float] NULL, "
		sqlCTH = sqlCTH & "[Data] [datetime] NULL, "
		sqlCTH = sqlCTH & "[Dependenta] [float] NULL, "
		sqlCTH = sqlCTH & "[Operacio] [nvarchar] (25) NULL "
		sqlCTH = sqlCTH & ") ON [PRIMARY]"
		rec sqlCTH
	end if
end function
...
function tablaDependentesExtes()
	...
	if not exists (tablaDependentesExtes) then
		sqlCTDE = "CREATE TABLE [dbo].[" & tablaDependentesExtes & "] ( "
		sqlCTDE = sqlCTDE & "[id] [nvarchar] (255) NULL , "
		sqlCTDE = sqlCTDE & "[nom] [nvarchar] (255) NULL , "
		sqlCTDE = sqlCTDE & "[valor] [nvarchar] (255) NULL "
		sqlCTDE = sqlCTDE & ") ON [PRIMARY]"
		rec sqlCTDE
	end if
end function
...
function tablaArticlesExtes()
	...
	if not exists (tablaArticlesExtes) then
		sqlCTAE = "CREATE TABLE [dbo].[" & tablaArticlesExtes & "] ( "
		sqlCTAE = sqlCTAE & "[id] [nvarchar] (255) NULL , "
		sqlCTAE = sqlCTAE & "[nom] [nvarchar] (255) NULL , "
		sqlCTAE = sqlCTAE & "[valor] [nvarchar] (4000) NULL "
		sqlCTAE = sqlCTAE & ") ON [PRIMARY]"
		rec sqlCTAE
	else
...
function tablaMoviments ( byval D )
   ...
   tablaMoviments = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Moviments_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaMoviments) then
		sqlCTM = "CREATE TABLE " & tablaMoviments & " ("
...
function tablaMovimentsEstat ( byval D )
   ...
   tablaMovimentsEstat = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Moviments_" & year(D) & "-" & right("00" & month(D),2) & "_Estat]"
   if not exists (tablaMovimentsEstat) then
		sqlCTM = "CREATE TABLE " & tablaMovimentsEstat & " ("
		SqlCTM = SqlCTM & "[Id] [nvarchar] (255) NULL Default (NEWID()), "
...
function tablaAlbarans (byval D)
	...
	tablaAlbarans = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[v_albarans_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaAlbarans) then
		sqlCTA = "CREATE TABLE " & tablaAlbarans & " ( "
		sqlCTA = sqlCTA & "[Botiga] [float] NULL, "
		sqlCTA = sqlCTA & "[Data] [datetime] NULL, "
		sqlCTA = sqlCTA & "[Dependenta] [float] NULL, "
		sqlCTA = sqlCTA & "[Num_tick] [float] NULL, "
		...
		sqlCTA = sqlCTA & "[Import] [float] NULL, "
		sqlCTA = sqlCTA & "[Tipus_venta] [nvarchar] (25) NULL, "
		sqlCTA = sqlCTA & "[FormaMarcar] [nvarchar] (255) NULL CONSTRAINT [DF__V_Albaran_" & year(D) & "-" & right("00" & month(D),2) & "_FM] DEFAULT (''), "
		sqlCTA = sqlCTA & "[Otros] [nvarchar] (255) NULL CONSTRAINT [DF__V_Albaran__" & year(D) & "-" & right("00" & month(D),2) & "_OT] DEFAULT ('') "
		sqlCTA = sqlCTA & ") ON [PRIMARY]"
		rec sqlCTA
	end if
end function
...
function tablaRevisats ( byval D )
	...
	tablaRevisats = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Revisat_" & year(D) & "-" & right("00" & month(D),2) & "]"
...
function tablaServits(byval D)
	dim Sql, SqlCTS, Nomtaula, Bak
	
	Nomtaula = "Servit-" & right(year(D),2) & "-" & right("00" & month(D),2) & "-" & right("00" & day(D),2) & ""	
	
	Bak = ""
	if Session("Data_BKP_Servit")<>"" then
		if cdate(Session("Data_BKP_Servit")) > cdate(D) then Bak = "_bak"
	end if
	
	tablaServits = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & NomTaula & "]"	
	
	if not exists (tablaServits) then

		SqlCTS = "CREATE TABLE " & tablaServits & " ("
		...
		Sql = "CREATE TABLE [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & NomTaula & "Trace] ("
		...
		rec  "DROP TRIGGER [M_" & Nomtaula & "] "

		Sql = "CREATE TRIGGER [M_" & Nomtaula & "] ON [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" &  Nomtaula & "] "
		Sql = Sql & "AFTER INSERT,UPDATE,DELETE AS "
		Sql = Sql & "Update [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "] Set [TimeStamp] = GetDate(),    [QuiStamp]  = Host_Name() Where Id In (Select Id From Inserted) "
		Sql = Sql & "Insert Into [" & session("Usuari_Empresa_Db") & Bak & "].dbo.ComandesModificades Select Id As Id,GetDate() As [TimeStamp],'" & Nomtaula & "' As TaulaOrigen From Inserted "
		Sql = Sql & "Insert into [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "Trace] (Id,[TimeStamp],[QuiStamp],Client,CodiArticle,PluUtilitzat,Viatge,Equip,QuantitatDemanada,QuantitatTornada,QuantitatServida,MotiuModificacio,Hora,TipusComanda,Comentari,ComentariPer,Atribut,CitaDemanada) Select Id,[TimeStamp],[QuiStamp],Client,CodiArticle,PluUtilitzat,Viatge,Equip,QuantitatDemanada,QuantitatTornada,QuantitatServida,MotiuModificacio,Hora,TipusComanda,Comentari,ComentariPer,Atribut,CitaDemanada  from [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "] Where Id In (Select Id From Inserted)"
		...
		Sql = Sql & "If Update (QuantitatTornada)  Update [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "] Set [CitaTornada]  = Cast(CAST(Host_Name() AS VarChar(255)) + ',' + CAST(QuantitatTornada  AS VarChar(255))  + ',' + CAST(GetDate() AS VarChar(255))  +  '/' + [CitaTornada]  AS VarChar(255)) Where Id In (Select Id From Inserted) "
		Sql = Sql & "If Update (QuantitatServida)  Update [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "] Set [CitaServida]  = Cast(CAST(Host_Name() AS VarChar(255)) + ',' + CAST(QuantitatServida  AS VarChar(255))  + ',' + CAST(GetDate() AS VarChar(255))  +  '/' + [CitaServida]  AS VarChar(255)) Where Id In (Select Id From Inserted) "
		Sql = Sql & "If Update (QuantitatDemanada) Update [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & Nomtaula & "] Set [CitaDemanada] = Cast(CAST(Host_Name() AS VarChar(255)) + ',' + CAST(QuantitatDemanada AS VarChar(255))  + ',' + CAST(GetDate() AS VarChar(255))  +  '/' + [CitaDemanada] AS VarChar(255)) Where Id In (Select Id From Inserted) "
		
		rec  Sql
	end if

	if not exists ("[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & NomTaula & "Trace]") then
		Sql = "CREATE TABLE [" & session("Usuari_Empresa_Db") & Bak & "].dbo.[" & NomTaula & "Trace] ("
		Sql = Sql & "[Modificat]         [datetime]              Default (GetDate())  ,"
		Sql = Sql & "[Id]                [nvarchar] (255) Null ,"
		Sql = Sql & "[TimeStamp]         [datetime]       Null ,"
...
function taulaTornats ( byval D )
   ...
   taulaTornats = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Tornat_" & year(D) & "-" & right("00" & month(D),2) & "]"
end function
...
function tablaLogs ( byval D )
	...
	tablaLogs = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Log_" & year(D) & "-" & right("00" & month(D),2) & "]"
end function
...
function tablaInventaris ( byval D )
	...
	tablaInventaris = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Inventaris_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists(tablaInventaris) then
		sqlCTI = "CREATE TABLE " & tablaInventaris & " ( "
		sqlCTI = sqlCTI & "[Botiga] [float] NULL, "
		sqlCTI = sqlCTI & "[Data] [datetime] NULL, "
		sqlCTI = sqlCTI & "[Dependenta] [float] NULL, "
		sqlCTI = sqlCTI & "[Num_tick] [float] NULL, "
		sqlCTI = sqlCTI & "[Estat] [nvarchar] (25) NULL, "
		sqlCTI = sqlCTI & "[Plu] [float] NULL, "
		sqlCTI = sqlCTI & "[Quantitat] [float] NULL, "
		sqlCTI = sqlCTI & "[Import] [float] NULL, "
		sqlCTI = sqlCTI & "[Tipus_venta] [nvarchar] (25) NULL "
		sqlCTI = sqlCTI & ") ON [PRIMARY]"
		rec sqlCTI
	end if
end function
...
function tablaFacturacioIva ( byval D )
	...
	tablaFacturacioIva = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[facturacio_" & year(D) & "-" & right("0" & month(D),2) & "_iva]"
...
function tablaFacturacioEditBakIva (  )	
	tablaFacturacioEditBakIva = "[" & session("Usuari_Empresa_Db") & "].dbo.[facturacioEditBak_iva]"
	if not exists ( tablaFacturacioEditBakIva ) then
		sqlFI = "CREATE TABLE " & tablaFacturacioEditBakIva & " ("
		sqlFI = sqlFI & "	IdFactura nvarchar (255) NULL,"
		sqlFI = sqlFI & "	NumFactura float NULL,"
		sqlFI = sqlFI & "	EmpresaCodi float NULL,"
		sqlFI = sqlFI & "	Serie nvarchar (255) NULL,"
		...
		sqlFI = sqlFI & "	IvaRec4 float NULL,"
		sqlFI = sqlFI & "	Reservat nvarchar (255) NULL,"
		sqlFI = sqlFI & "	TimeStamp datetime NULL"		
		sqlFI = sqlFI & "	)"
		rec sqlFI
	end if
end function
...
function tablaFacturacioData ( byval D )
	...
	tablaFacturacioData = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[facturacio_" & year(D) & "-" & right("0" & month(D),2) & "_Data]"
...
function tablaFacturacioEditBakData (  )
	tablaFacturacioEditBakData = "[" & session("Usuari_Empresa_Db")  & "].dbo.[facturacioEditBak_Data]"
	if not exists ( tablaFacturacioEditBakData ) then
		sqlFD = "CREATE TABLE " & tablaFacturacioEditBakData & " ("
		sqlFD = sqlFD & "	IdFactura nvarchar (255) NULL,"
		sqlFD = sqlFD & "	Data datetime NULL,"
		sqlFD = sqlFD & "	Client float NULL,"
		sqlFD = sqlFD & "	Producte float NULL,"
		...
		sqlFD = sqlFD & "	Servit float NULL,"
		sqlFD = sqlFD & "	Tornat float NULL,"
		sqlFD = sqlFD & "	TimeStamp datetime NULL"
		sqlFD = sqlFD & "	)"
		rec sqlFD
	end if
end function
...
function tablaPressupostData ( byval D )
	...
	tablaPressupostData = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[pressupost_" & year(D) & "-" & right("0" & month(D),2) & "_Data]"
...
function tablaPressupostIva ( byval D )
	...
	tablaPressupostIva = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[pressupost_" & year(D) & "-" & right("0" & month(D),2) & "_iva]"
...
function tablaFacturacioReb ( byval D )
	...
	tablaFacturacioReb = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[facturacio_" & year(D) & "-" & right("0" & month(D),2) & "_reb]"
...
function tablaPressupostReb ( byval D )
	...
	tablaPressupostReb = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[pressupost_" & year(D) & "-" & right("0" & month(D),2) & "_reb]"
...
function tablaEstadisticas ( byval D )
	...
	tablaEstadisticas = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[V_Estadis_" & year(D) & "-" & right("00" & month(D),2) & "]"
	if not exists (tablaEstadisticas) then  
		sqlE = "CREATE TABLE " & tablaEstadisticas & " ("
...
function tablaMateriasPrimas ( )
	...
	if not exists (tablaMateriasPrimas) then
		...
		sqlMP = sqlMP & "[precioFormato] [numeric](18, 4) NULL ,"
		sqlMP = sqlMP & "[iva] [float] NULL "
		sqlMP = sqlMP & ") ON [PRIMARY]"
		rec sqlMP

		sqlMP = "insert into ccMateriasPrimasZombis  ([id],[nombre],[descripcion],[alta],[activo],[codigo],[caducidad],[refrigeracion],[etiquetado],[barcode],[envasado],[unidades],[almacen],[etiqueta],[proveedor],[tipoEnvio],[nevera],[ubicacion],[unidadesEnvio],[temperatura],[stockMin],[precio],[precioFormato],[iva]) "
		sqlMP = sqlMP & ") select [id],[nombre],[descripcion],[alta],[activo],[codigo],[caducidad],[refrigeracion],[etiquetado],[barcode],[envasado],[unidades],[almacen],[etiqueta],[proveedor],[tipoEnvio],[nevera],[ubicacion],[unidadesEnvio],[temperatura],[stockMin],[precio],[precioFormato],[iva] "
...
function tablaPedidos ( )
	...
	if not exists (tablaPedidos) then
		...
		sqlP = sqlP & "[activo] [bit] NULL CONSTRAINT [DF_" & tablaPedidos & "_activo] DEFAULT (1), "
		sqlP = sqlP & "[confirmado] [bit] NULL CONSTRAINT [DF_" & tablaPedidos & "_confirmado] DEFAULT (0) ) ON [PRIMARY] "
		rec sqlP
	end if
end function
...
function taulaComptabilitzats ( byval D )
	...
	taulaComptabilitzats = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[Comptabilitzats_" & year(D) & "-" & right("00" & month(D),2) & "]"
...
function tablaEncarrecs ( byval D )
	...
	tablaEncarrecs = "[" & session("Usuari_Empresa_Db") & Bak & "].dbo.[v_encarre_" & year(D) & "-" & right("0" & month(D),2) & "]"
...
function tablaFichasInformativas ( )
	dim sqlFI
	tablaFichasInformativas = "fichasInformativas"
	if not exists ( tablaFichasInformativas ) then
		sqlFI = "create table " & tablaFichasInformativas & " ( "
		sqlFI = sqlFI & "[Id]       [nvarchar] (255) NULL constraint [DF_" & tablaFichasInformativas & "_id] default (newid()), "
		sqlFI = sqlFI & "[nom] 		[nvarchar](255) NULL, "
		sqlFI = sqlFI & "[fInicio]  [datetime] NULL CONSTRAINT [DF_" & tablaFichasInformativas & "_fInicio] DEFAULT (getdate()), "
		sqlFI = sqlFI & "[fFin]     [datetime] NULL CONSTRAINT [DF_" & tablaFichasInformativas & "_fFin] DEFAULT (getdate()) "
		sqlFI = sqlFI & ") ON [PRIMARY] "		
		rec sqlFI
	end if
end function

function tablaFichaInformativaPDF ( )
	dim sqlFI
	tablaFichaInformativaPDF = "fichaInformativaPDF"
	if not exists ( tablaFichaInformativaPDF ) then
		sqlFI = "create table " & tablaFichaInformativaPDF & " ( "
		sqlFI = sqlFI & "[Id]          [nvarchar] (255) NULL constraint [DF_" & tablaFichaInformativaPDF & "_id] default (newid()), "
		sqlFI = sqlFI & "[IdFicha]     [nvarchar] (255) NULL , "
		sqlFI = sqlFI & "[DocAdjunto]  [nvarchar] (255) NULL, "
		sqlFI = sqlFI & "[Descripcion] [nvarchar](255) NULL "
		sqlFI = sqlFI & ") ON [PRIMARY] "		
		rec sqlFI
	end if
end function

function tablaFichaInformativaTiendas ( )
	dim sqlFI
	tablaFichaInformativaTiendas = "fichaInformativaTiendas"
	if not exists ( tablaFichaInformativaTiendas ) then
		sqlFI = "create table " & tablaFichaInformativaTiendas & " ( "
		sqlFI = sqlFI & "[Id]          [nvarchar] (255) NULL constraint [DF_" & tablaFichaInformativaTiendas & "_id] default (newid()), "
		sqlFI = sqlFI & "[IdFicha]     [nvarchar] (255) NULL , "
		sqlFI = sqlFI & "[CodiBotiga]  [int]  NULL, "
		sqlFI = sqlFI & "[Descripcion] [nvarchar](255) NULL "
		sqlFI = sqlFI & ") ON [PRIMARY] "		
		rec sqlFI
	end if
...
function tablaAuditoriaTPV ( )
	dim sqlA
	tablaAuditoriaTPV = "auditoriaTPV"
	if not exists ( tablaAuditoriaTPV ) then
		sqlA = "create table " & tablaAuditoriaTPV & " ( "
		sqlA = sqlA & "[Id]       [nvarchar] (255) NULL constraint [DF_" & tablaAuditoriaTPV & "_id] default (newid()), "
		sqlA = sqlA & "[Pregunta] [nvarchar](255) NULL, "
		sqlA = sqlA & "[fInicio]  [datetime] NULL CONSTRAINT [DF_" & tablaAuditoriaTPV & "_fInicio] DEFAULT (getdate()), "
		sqlA = sqlA & "[fFin]     [datetime] NULL CONSTRAINT [DF_" & tablaAuditoriaTPV & "_fFin] DEFAULT (getdate()), "
		sqlA = sqlA & "[Puntuacion] [int] NULL "
		sqlA = sqlA & ") ON [PRIMARY] "		
		rec sqlA
	end if
end function

function tablaAuditoriaRespuestasTPV ( )
	dim sqlA
	tablaAuditoriaRespuestasTPV = "auditoriaRespuestasTPV"
	if not exists ( tablaAuditoriaRespuestasTPV ) then
		sqlA = "create table " & tablaAuditoriaRespuestasTPV & " ( "
		sqlA = sqlA & "[IdAuditoria] [nvarchar] (255) NULL, "
		sqlA = sqlA & "[Fecha]       [datetime] NULL CONSTRAINT [DF_" & tablaAuditoriaRespuestasTPV & "_fecha] DEFAULT (getdate()), "
		sqlA = sqlA & "[CodiBotiga]  [int]  NULL, "		
		sqlA = sqlA & "[Puntuacion]  [int] NULL "
		sqlA = sqlA & ") ON [PRIMARY] "		
		rec sqlA
	end if
...
function tablaAuditoriaTiendas ( )
	dim sqlFI
	tablaAuditoriaTiendas = "auditoriaTiendas"
	if not exists ( tablaAuditoriaTiendas ) then
		sqlFI = "create table " & tablaAuditoriaTiendas & " ( "
		sqlFI = sqlFI & "[Id]          [nvarchar] (255) NULL constraint [DF_" & tablaAuditoriaTiendas & "_id] default (newid()), "
		sqlFI = sqlFI & "[IdAuditoria]     [nvarchar] (255) NULL , "
		sqlFI = sqlFI & "[CodiBotiga]  [int]  NULL, "
		sqlFI = sqlFI & "[Descripcion] [nvarchar](255) NULL "
		sqlFI = sqlFI & ") ON [PRIMARY] "		
		rec sqlFI
...
function tablaParamsEquip()
	tablaParamsEquip = "paramsEquip"
	if not exists (tablaParamsEquip) then
		sqlCTAE = "CREATE TABLE [dbo].[" & tablaParamsEquip & "] ( "
		sqlCTAE = sqlCTAE & "[equip] [nvarchar] (255) NULL , "
		sqlCTAE = sqlCTAE & "[param] [nvarchar] (255) NULL , "
		sqlCTAE = sqlCTAE & "[valor] [nvarchar] (4000) NULL "
		sqlCTAE = sqlCTAE & ") ON [PRIMARY]"
		rec sqlCTAE
	end if
...
function tablaFamiliesExtes()
	tablaFamiliesExtes = "familiesExtes"
	if not exists (tablaFamiliesExtes) then
		sqlCTDE = "CREATE TABLE [dbo].[" & tablaFamiliesExtes & "] ( "
		sqlCTDE = sqlCTDE & "[familia] [nvarchar] (255) NULL , "
		sqlCTDE = sqlCTDE & "[variable] [nvarchar] (255) NULL , "
		sqlCTDE = sqlCTDE & "[valor] [nvarchar] (255) NULL "
		sqlCTDE = sqlCTDE & ") ON [PRIMARY]"
		rec sqlCTDE
	end if
