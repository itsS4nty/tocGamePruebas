...
<%
	...
	if rs.eof then set Rs = recHit("select * from web_empreses Where nom = 'daza' " )
	...
	if calLote  then
		OrdreArticles = " Hora,cast(isnull(o.valor,9999) as numeric) "	
	else
...
sub activateRuleState(ruleName)
	...
	if not CalLote then activateRule "hora"
...
'	if ruleName = "pedir lote data" then CargaLotesArtticulo
...
'	wend
	...
	if session("TipoReparto") = "equipo" then 
		Set Rs = rec("Select " & EnviaViatges & " Distinct Equip Nom From " & tablaServits(Dia) & " where not Equip = '' order by Equip")
		Clau = "Equipo"
	else
		Set Rs = rec("Select " & EnviaViatges & " Distinct Viatge Nom From " & tablaServits(Dia) & " where not Viatge = '' order by Viatge ")
		Clau = "Reparto" 
	end if	
	
	session("CantaListaTodos")  = ""
	while not rs.eof
		nom = XmlOk(lcase(neteja(rs("Nom"))))
		session("CantaListaTodos") = session("CantaListaTodos") & "," &  nom
		str = "<P VALSTR='" & nom & "'>"
		str = str & "<P>" & Clau & " " & Neteja(Nom) & "</P><P>Fin</P>"
		str = str & "</P>"
		appendXML2 str, L_nombre
		rs.movenext
	wend
...
function StartEventLogin(NextState) 
	dim Rs
	...
	set L_nombre = speechGram.selectSingleNode("/GRAMMAR/RULE[@NAME='pedir nombres']/L")
	Set Rs = rec("select Codi,memo as Nom From Dependentes Where not nom = '' Order By Nom ")
	session("CantaListaTodos")  = ""
	while not rs.eof
		Nom = XmlOk(lcase(rs("Nom")))
		Nom = join(split(Nom,"_web"),"")
		session("CantaListaTodos") = session("CantaListaTodos") & "," &  nom
		Nom = Canvia(Nom,","," ")
		n = split(nom," ")
		str = "<P VALSTR='" & rs("Codi") & "'>"
...
function CargaLotesArtticulo() 
	...
	set L_nombre = speechGram.selectSingleNode("/GRAMMAR/RULE[@NAME='pedir lote data']/L")
	Article = ArticleIdAlbaraCodiA(session("IdAlbara")) 
	Set Rs = rec("select nom from applots  where plu = '" & Article  & "'  and datediff(dd,datai,getdate()) < 12 ")
	session("CantaListaTodos")  = ""
	while not rs.eof
		Nom = XmlOk(lcase(rs("Nom")))
		session("CantaListaTodos") = session("CantaListaTodos") & "," &  nom
...
'			if Temperatura >  10 then speak ", Demasiado alta , llama a Cesar , 636 95 42 73" 
			...
			activateRuleState "menu"
		case "OrdenSiguiente","OrdenAnterior"
'			rec "insert into ordenreparto (codi,tipus,reparto,nom) select codi,'Article','" & session("Reparto Actual") & "',left(nom,20) from  (select cc.*,Orden  from Articles cC left join ordenreparto o on o.codi = cc.codi and tipus='Article' and reparto = '" & session("Reparto Actual") & "') c where  not codi in (Select codi from Ordenreparto Where tipus='Article' and reparto = '" & session("Reparto Actual") & "' )  order by orden  "
'			BuscaOrdenReparto "Article",ArticleIdAlbaraCodiA(session("IdAlbara")),Ord,OrdSigg,OrdAnt
			OrdSw=OrdSigg
			if rule = "OrdenAnterior" then OrdSw=OrdAnt
			if OrdSw > 0 then
'				rec "Update ordenreparto Set Codi = " & ArticleIdAlbaraCodiA(session("IdAlbara")) & "  Where tipus = 'Article' And Reparto = '" & session("Reparto Actual") & "' and Codi =" & OrdSw
'				rec "Update ordenreparto Set Codi = " & OrdSw               & "  Where tipus = 'Article' And Reparto = '" & session("Reparto Actual") & "' and Orden =" & Ord
'				BuscaOrdenReparto "Article",ArticleIdAlbaraCodiA(session("IdAlbara")),Ord,OrdSigg,OrdAnt
				NomOrdAnt  = ProductoNombre(OrdAnt)
				if OrdAnt = 0 then NomOrdAnt = "Inicio "
				NomOrdSigg = ProductoNombre(OrdSigg)
				if OrdSigg = 0 then NomOrdSigg = "Fin "
				speak "Nuevo Orden " & NomOrdAnt & "  " & ProductoNombre(ArticleIdAlbaraCodiA(session("IdAlbara")))  & "  " & NomOrdSigg & " Actual Mente "
			else
				speak "Canvio Imposible"
			end if	
			ProductoReparte
...
'				speak "El trim es  " & trim(nombre)  &  " todos " 
			...
			if session("NextState") = "IniciaReparto" then	
				IniciaReparto
			else
				activateRuleState session("NextState")
			end if
		...
		case "menu" :
			...
			select case menu
				...
				case "ruta","equipo" :
					session("TipoReparto") = menu
					if session("nombreConfirmar") = "" then
						StartEventLogin "IniciaReparto"
					else
						IniciaReparto
					end if	
				case "Fecha"					
		...
		case "navega cliente" :
			select case getProp("navega cliente")
				...
				case "Todos"
					...
					ComercioCanta
				case "OrdenSiguiente","OrdenAnterior"
'					rec "insert into ordenreparto (codi,tipus,reparto,nom) select codi,'Client','" & session("Reparto Actual") & "',left(nom,20) from  (select cc.*,Orden  from Clients cC left join ordenreparto o on o.codi = cc.codi and tipus='Client' and reparto = '" & session("Reparto Actual") & "') c where  not codi in (Select codi from Ordenreparto Where tipus='Client' and reparto = '" & session("Reparto Actual") & "' )  order by orden  "
'					BuscaOrdenReparto "Client",session("Comercio"),Ord,OrdSigg,OrdAnt
					OrdSw=OrdSigg
					if getProp("navega cliente") = "OrdenAnterior" then OrdSw=OrdAnt
					if OrdSw > 0 then
'						rec "Update ordenreparto Set Codi = " & session("Comercio") & "  Where tipus = 'Client' And Reparto = '" & session("Reparto Actual") & "' and Codi =" & OrdSw
'						rec "Update ordenreparto Set Codi = " & OrdSw               & "  Where tipus = 'Client' And Reparto = '" & session("Reparto Actual") & "' and Orden =" & Ord
'						BuscaOrdenReparto "Client",session("Comercio"),Ord,OrdSigg,OrdAnt
						NomOrdAnt  = ClienteCodiNom(OrdAnt)
						if OrdAnt = 0 then NomOrdAnt = "Inicio "
						NomOrdSigg = ClienteCodiNom(OrdSigg)
						if OrdSigg = 0 then NomOrdSigg = "Fin "
						speak "Nuevo Orden " & NomOrdAnt & "  " & ClienteCodiNom(session("Comercio"))  & "  " & NomOrdSigg & " Actual Mente "
					else
						speak "Canvio Imposible"
					end if	
					ComercioCanta
...
function comercioNombre(CodiComercio) 
	...
	if not rs.eof then comercioNombre = neteja(rs("Nom")) 
end function
...
function ClienteCodiNom(Codi) 
    ...
    if not rs.eof then ClienteCodiNom = Neteja(rs("Nom"))
end function
...
Function BuscaOrdenReparto (Tipus,C,Ord,OrdSig,OrdAnt)
	Ord=0
	OrdSig=0
	OrdAnt=0
	Set Rs = rec("select * from ordenreparto  where tipus = '" & Tipus & "' and Reparto = '" & session("Reparto Actual") & "' and codi = " & C & " ")
    if not rs.eof then Ord = rs("Orden")

	Set Rs = rec("select distinct c.codi,Orden from ordenreparto c join " & tablaServits(Dia) & " s on c.codi = s.client  and  " & session("Reparto Actual Condicio") & "  where tipus = '" & Tipus & "' and Reparto = '" & session("Reparto Actual") & "' and Orden > " & Ord & " Order By Orden ")
    if not rs.eof then OrdSig = rs("Codi")
	Set Rs = rec("select distinct c.codi,Orden from ordenreparto c join " & tablaServits(Dia) & " s on c.codi = s.client  and  " & session("Reparto Actual Condicio") & " 'where tipus = '" & Tipus & "' and Reparto = '" & session("Reparto Actual") & "' and Orden < " & Ord & " Order By Orden Desc ")
    if not rs.eof then OrdAnt = rs("Codi")
end function
...
sub ProductoReparte()
	...
	if len(comentari) >0 then  speak "Comentario " &  comentari & " . "
	
	speak ProductoNombre(ArticleIdAlbaraCodiA(session("IdAlbara"))) & ". " & session("Ordinal Peso Pedido")
	activateRule "Producto Siguiente"
	activateRule "Producto Anterior"
	activateRule "FinAlbaran"
'	activateRule "OrdenSiguiente"
'	activateRule "OrdenAnterior"
...
function ProductoNombre(Codi) 
	...
	dim Rs
	Set Rs = rec("select Nom from Articles Where Codi = " & Codi)
	if not rs.eof then ProductoNombre = Neteja(rs("Nom"))  ' & " , 
end function
...
sub productoConfirmado() 
	...
	rec "Update " & tablaServits(Dia) & " Set Comentari = Comentari + '[Lote:" & lote & "][Cad:10-02-09]' ,QuantitatServida = '" & peso & "' Where Id = '" & session("IdAlbara") & "' "  
	rec "Insert Into CalEnviar (Id,Taula,TmSt) Values ('" & session("IdAlbara") & "','" & tablaServits(Dia) & "',GetDate()) " 
