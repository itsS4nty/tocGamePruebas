...
sub ComercioMueve(posicion) 
	dim Rs
	Set Rs = rec("select Distinct c.Codi codi ,Nom,cast(isnull(cc.valor,0) as numeric) from Clients c join " & tablaServits(Dia) & " s on c.codi = s.client left join constantsclient cc on isnumeric(cc.valor)= 1 and cc.codi = c.codi and cc.variable = 'Ruta_Orden'  Where Viatge = '" & session("Reparto Actual") & "' " & session("FiltreLots") & " order by cast(isnull(cc.valor,0) as numeric) ,nom ")
