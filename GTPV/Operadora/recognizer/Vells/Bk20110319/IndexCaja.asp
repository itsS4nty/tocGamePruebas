...
function Neteja(s)
	...
	while not RsNet.eof
		if instr(s,RsNet("Dije")) > 0 or instr(s,lcase(RsNet("Dije"))) > 0  then 
'speak "Antes " & RsNet("Dije") & " i ahora " & RsNet("Digo")		
...
'	wend
	...
	while not rs.eof
		...
		str = "<P VALSTR='" & nom & "'>"
		str = str & "<P>Reparto " & Nom & "</P><P>FIN</P>"
		str = str & "</P>"
		appendXML2 str, L_nombre
		rs.movenext
	wend
...
sub ComercioMueve(posicion) 
	dim Rs
	Set Rs = rec("select Distinct c.Codi codi ,Nom,cast(isnull(cc.valor,isnull(ccc.valor,c.codi)) as numeric) from Clients c join " & tablaServits(Dia) & " s on c.codi = s.client left join constantsclient cc on isnumeric(cc.valor)= 1 and cc.codi = c.codi and cc.variable = 'Ruta_Orden' left join constantsclient ccc on isnumeric(ccc.valor)= 1 and ccc.codi = c.codi and ccc.variable = 'CodiContable' Where Viatge = '" & session("Reparto Actual") & "' " & session("FiltreLots") & session("OrdenClientes") )
...
sub ProductoMueve(posicion) 
	dim Rs
	Set Rs = rec("select Nom,Codi,Id from Articles c join " & tablaServits(Dia) & " s on c.codi = s.CodiArticle Where " & CondicionCliente & " s.viatge = '" & session("Reparto Actual") & "' " & session("FiltreLots") & " order by nom ")
...
function EsBof(Taula) 
	...
	if Taula = "IdAlbara" then
		Set Rs = rec("select Id,Nom,Codi from Articles c join " & tablaServits(Dia) & " s on s.Client = '" & session("Comercio") & "' order by nom ")
...
function EsEof(Taula) 
	...
	if Taula = "IdAlbara" then
		Set Rs = rec("select Nom,Codi,Id from Articles c join " & tablaServits(Dia) & " s on c.codi = s.CodiArticle Where " & CondicionCliente & " s.viatge = '" & session("Reparto Actual") & "' " & session("FiltreLots") & " order by nom ")
		if session(Taula) = MueveRs (Rs,"Siguiente",Taula) then 	
'			speak "select Nom,Codi,Id from Articles c join " & tablaServits(Dia) & " s on c.codi = s.CodiArticle Where " & CondicionCliente & " s.viatge = '" & session("Reparto Actual") & "' " & session("FiltreLots") & " order by nom "
'			speak "[" & session(Taula) &  "]"
			EsEof = true
		end if	
	else
