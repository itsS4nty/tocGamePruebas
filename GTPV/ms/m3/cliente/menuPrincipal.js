H.Scripts.add("menuPrincipalS", "L2", function(window) {
...
//	var TaulaActual;
	...
	var paramsMenuScroll = {
		...
		getItem : function(idx) {
			...
			var dataOp = my.menus.ops[op];
			var el = menuModel.clone(true).data("data", op);	
			if (dataOp.cssClass != null) el.addClass(dataOp.cssClass);
			if (dataOp.image != null) {
				$("<img>").attr("src", dataOp.image)
						  .css({ paddingLeft: "8px", height: "100%", "float": "left"}).appendTo(el);
			}
			$("<img>").attr("src", "css/gtpv/img/dummy.png")
			          .css({ height : "100%", "verticalAlign" : "middle", width: "0px", visibility: "hidden", }).appendTo(el);
			
			$("<span>").text(M(dataOp.text)).css("verticalAlign", "middle").appendTo(el);
			if (op === S.opMenuSel) el.addClass("g-state-active");
			return el;
		},
		nElScroll : 2,
	};

	var menuModel = $("<button>").width("100%").css("textAlign", "center").addClass("g-state-default");
	...
	*/
	...
	var dependentesActivesModel = $("<button>").addClass("g-state-default");
...
*/
		for (var opName in my.menus.ops) {
			my.menus.ops[opName].init(layout.content);
