...
H.ImageFiles = function() {
	...
	function handlerWeb(path, idSat, byName) {
		...
		return function(sendResponseFile) {
			var i = $("<img>").attr("src", path.slice(1));
			i.load(function() {
				var c = $("<canvas>");
				var c0 = c[0], i0 = i[0];
				c0.width = i0.naturalWidth || i0.width;
				c0.height = i0.naturalHeight || i0.height;
				c.appendTo("body");
				c0.getContext("2d").drawImage(i0, 0, 0);
				...
				var content = dataUrl.slice(idx+1);
				content = atob(content);
				sendResponseFile(my.add(path, content, (idSat != null), type));
			});
			i.error(function() {
				i.remove();
				sendResponseFile(null);
