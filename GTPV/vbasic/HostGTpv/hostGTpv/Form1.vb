...
Public Class Form1
    ...
    Private Shared sortedDeps() As H_Dependentes.CDependenta
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim j = satServer.getSiteLocalAddresses()
        For Each addr In j
            ListLocalAddress.Items.Add(addr)
        Next
        ...
        AddHandler H_Caja.evCerrar, Sub()
                                    ...
                                    End Sub
        '  dependentes
        sortedDeps = H_Dependentes.getDependentes()
        ListDep.Items.Clear()
        For Each d In sortedDeps
        ...
        AddHandler ListDep.ItemCheck, Sub(sender2 As Object, e2 As ItemCheckEventArgs)
                                          ...
                                          End If
                                      End Sub
        AddHandler ListDep.SelectedIndexChanged, Sub(sender2 As Object, e2 As EventArgs)
                                                     Dim codi = sortedDeps(ListDep.SelectedIndex).codi
                                                     Dim id = "D_" & codi
                                                     If H_Comandes.getComanda(id) Is Nothing Then
                                                         Dim c = New H_Comandes.CComanda(id)
                                                         c.fOpen = True
                                                         H_Comandes.actualizeComanda(id, c)
                                                     End If
                                                 End Sub

        '  mesas
        ListMesas.Items.Clear()
        For Each comedor In comedores
            For Each mesa In comedor.mesas
                ListMesas.Items.Add(mesa.name)
            Next
        Next
        AddHandler ListMesas.SelectedIndexChanged, Sub(sender2 As Object, e2 As EventArgs)
                                                       Dim id = "M_" & ListMesas.SelectedItem
                                                       If H_Comandes.getComanda(id) Is Nothing Then
                                                           Dim c = New H_Comandes.CComanda(id)
                                                           c.fOpen = True
                                                           H_Comandes.actualizeComanda(id, c)
